...
From Jim Decker on the monotone mailing list:

Mercurial merges can only be done through a workspace.

In Mercurial merge must be done through a workspace, so in order to
just merge two heads, If I had work in progress in the tree, I have to
clone the repository to another directory, check out the source(ya
that's the same step), and then do the merging; A commit from a merge
cannot be done with relative paths like 'I want to commit just this
part of the tree', you have to commit the whole thing, so work in
progress changes would get mixed up in the whole thing.  The revisions
to merge are all in the repository, so why does it require a
workspace?!  Git does the same thing in this respect.

\noindent{\bf Advantage:} likely monotone
...
and revert didn't work.

\noindent{\bf Advantage:} likely monotone

\subsection*{Pulling code remotely}

Both monotone and mercurial provide a 'clone' operation for this
purpose.  There is no noticeable difference between monotone and
mercurial.
...

{
  ...
  \begin{tabular}{|l|l|}  \hline
   {\bf In monotone}                                        & {\bf In mercurial}    \\ \hline
   {\tt \$ mtn heads –branch=<branch \#>} & {\tt \$ hg heads -b} \\ \hline
  \end{tabular}
  \vspace{5mm}
}

\noindent{\bf Advantage:} appears even
...

{
  ...
  \begin{tabular}{|l|l|}  \hline
   {\bf In monotone}                                                 & {\bf In mercurial}                                           \\ \hline
   {\tt \$ mtn serve –bind=<address>:<port \#>} & {\tt \$ hg serve -a <address> -p <port \#>} \\ \hline
  \end{tabular}
}
...

{
  ...
  \begin{tabular}{|l|l|}  \hline
   {\bf In monotone}                                                 & {\bf In mercurial}                                                          \\ \hline
   {\tt .$^*$Makefile\$}                                             & {\tt syntax:regex}                                                          \\
   {\tt .$^*$Makefile\textbackslash.in\$}               & {\tt .$^*$Makefile\textbackslash.in\$}                       \\
   {\tt .$^*$\textbackslash.output\$}                      & {\tt .$^*$\textbackslash@\textbackslash.[ch]}          \\
   {\tt .$^*$\textbackslash@\textbackslash.[ch]} & {\tt  \^{}aclocal\textbackslash.m4\$}                          \\
   {\tt  \^{}aclocal\textbackslash.m4\$}                  &                                                                                         \\
                                                                                   & {\tt syntax: glob}                                                            \\
                                                                                   & {\tt $^*$Makefile}                                                          \\ 
                                                                                   & {\tt $^*$.output}                                                             \\ \hline
  \end{tabular}
...
\noindent{\bf Advantage:} mercurial (slightly)
