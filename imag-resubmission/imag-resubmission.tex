...
subcellular, cellular, or network scales, but they are also restricted
to these scales.  The monolithic nature of these simulators makes it
almost impossible to extend the functionalities of these simulators
beyond the levels they support.

In contrast, the new modular structure and the communication module of
G-3's are key to its extensible multi-scale capabilities (described in
more detail below).
...
The modular structure allows for the rapid incorporation of new
solvers at a specific scale (see below for an example) while the
communication module allows to implement different types of run-time
...
extensible to meet new user needs.  At its essence, these two new
features of G-3 allow to transform multi-scale modeling in
neuroscience from today's specific implementations to a systematic
technique with a solid software foundation.
