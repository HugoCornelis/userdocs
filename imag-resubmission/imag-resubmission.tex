...
modular structure of G-3, particularly its novel communication module,
are key to the extensibility of multi-scale simulation capabilities.
Firstly it allows for rapid incorporation of new solvers responsible
for specific scales (see below for an example) and secondly, in
compliance with the G-3 software architecture and its facilitated
extensibility, scale-separation between solvers is maintained through
the provision of a module that supports the implementation of
different types of run-time interfaces between any concurrently active
solvers.
%the
...
specifically designed from the \underline{outset} to support modeling
...
a feature essential to the implementation of scale-linking.

As a simple use case of the addressing algorithm the Discrete Event
...
computational aspects of this project involve implementation of the
tools to expand the support to cross link and embed finer scale simulation
in cellular and network level models.

\subsubsection*{B.3 The new ``Scale-link'' functionality.}

\noindent Over\marginpar{expand computer science background, describe innovation
