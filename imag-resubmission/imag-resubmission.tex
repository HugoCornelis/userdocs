...
beyond the levels they explicitly support. In contrast, the new modular structure of G-3, particularly its novel communication module, are key to the extensibility of multi-scale simulation capabilities (described in
more detail below).
...
methods capture the macroscale behaviour of a system by simulation
...
distinction locates them in different modules of the G-3 simulator
architecture.

The modular software structure allows for rapid incorporation of new
solvers for responsible for specific scales (see below for an example) while,
...
continuous time by providing an interface between mathematical solvers and event queues that links simulation objects, running at their own specific scale, across the different model scales.

The existence of
a single model-container that integrates a model across multiple spatial scales
during the model construction phase is essential for the implementation of scale-linking.  When combined with an addressing algorithm in the
model-container the addresses of
solved variables can be obtained independently of their
scale from different solvers. \marginpar{this seems written upside down} This functionality
supports the instantiation of efficient communication components that can embed any required numerical scheme or algorithm necessary for simulation of
an aspect of the integrated model. 

As described in the next section,
the most innovative and challenging technical aspect of this
project\marginpar{way to many occurences of 'this project'} involves implementation of the tools to support cross linking and embedding of finer scale simulation
...
of G-3 supports this kind of functionality, at present there is only support for expansion
  of the scope of the upper or biological layer (see Fig.\,\ref{fig:cbi-architecture-expanded}). In the current NIH grant, there is neither direct
support for expansion of the scope
  of the lower or mathematical layer (see Fig.\,\ref{fig:cbi-architecture-expanded}) nor the associated development required for support of
multi-scale modeling. Instead that grant is
...
there is no overlap between these two projects. This new project will,
...
Before considering our approach to each of the Specific Aims
...
the redesign, the fundamentally modular structure of G-3 now supports not only the
...
delineation of scope for new module development\marginpar{unclear
  how the workflows do this, explain the workflows as complementary to
  the CBI architecture}, including the addition of functionality operating
...
Having described the new features of G-3 that make this
project possible, in this section we describe our specific
...
limitations in space, we do not describe the G-3 implementation of
...
\paragraph{C.2.1. Sub-cellular level modeling.} Key to the success of this project is the expansion of tools for constructing sub-cellular level models. Accordingly, as a test of the new multi-scale modeling capacity of G-3 we began working in May of 2011 with Dr. Avrama Blackwell (now a collaborator on this grant) to incorporate the core functionality of ``Chemesis'' (Blackwell 2000, Blackwell and Kotleski 2002) into G-3. ``Chemesis'' which supports the modeling of biochemical reactions, second messengers, and calcium dynamics (Blackwell, 2000) was originally added on to GENESIS version 2.0 by adding a user-contributed library over multiple years. In contrast, the core features of ``Chemesis-3'' were added to G-3 in under 3 months. It now runs almost three times faster than in GENESIS-2. This rapid and successful incorporation was a result of several explicit features of G-3, including: 1) the modular separation of the development of the core of new solvers from that of other software components, allowing full focus on the mathematical aspects of the solver and their implementation, and better optimization; 2) the G-3 Developer package facilitated integration of the ``Chemesis-3'' solver into the build system of G-3 enabling immediate regression tests; 3) easy extension of the configuration of the model-container to establish new declarative model tokens and parameters recognized by the existing ``Chemesis-3'' solvers; 4) easy construction of an interface between the model-container application programmers interface (API) and the core of the ``Chemesis-3'' solver, and finally 5) the specific development of a software module that interfaces the new solver with the existing linear cable solver. In addition to the further development of ``Chemesis-3'', we will use the same G-3 developer functionality to add a biochemical kinetic modeling system being developed for MOOSE (Bhalla and Iyengar, 1999; Bhalla, 2002; Sivakumaran, et al., 2003; Ray and Bhala, 2008). \\
...
existing and new G-3 solvers can be integrated to simulate
