...
A first important aspect of G-3's multi-scale capacity is the
existence of a single ``model-container'' that stores a model across
multiple spatial scales during the model construction phase.  An
``addressing algorithm'', provided by the ``model-container'', allows
solvers to obtain the run-time addresses of variables of other solvers
...
components that can embed any required numerical scheme or algorithm
necessary for upscaling or downscaling variables or other aspects of
