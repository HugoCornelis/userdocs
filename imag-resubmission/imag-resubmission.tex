...
\begin{figure}
\centering
\includegraphics[scale=0.4]{figures/collaboration-structure.eps}
...
% It is a feature that is essential to the implementation of
% scale-linking in G-3.
...
different model scales (Johan Eker et al. 2003).
% Johan Eker, Jorn Janneck, Edward A. Lee, Jie Liu, Xiaojun Liu, Jozsef Ludvig, Sonia Sachs, Yuhong Xiong. Taming heterogeneity - the Ptolemy approach, Proceedings of the IEEE, 91(1):127-144, January 2003.
%, that links simulation objects running at their
%own specific scale.

%These changes are briefly described in the
...
%software development that supports multi-scale modeling.
%Furthermore,
...
%At present many neuroscience simulators, including GENESIS-2, allow
...
%  M. Ainsworth and J. T. Oden, A Posteriori Error Estimation in Finite Element Analysis, Wiley-Interscience, NY, 2000.
and wavelet based methods (I. Daubechies, 1992),
%  I. Daubechies, Ten Lectures on Wavelets, CBMS-NSF Regional Conference Series in Applied Mathematics, SIAM Publications, 1992.
...
% E.B. Tadmor, M. Ortiz and R. Phillips, Quasicontinuum Analysis of Defects in Crystals, Philos. Mag., A73 , 1529--1563, 1996.
heterogeneous multi-scale modeling (Weinan. E and B. Engquist, 2003)
%  Weinan. E and B. Engquist, The Heterogeneous Multi-Scale Methods, Comm. Math. Sci., vol. 1, pp. 87--133, 2003.
and adaptive model refinement (A.L. Garcia et al, 1999)
%  A.L. Garcia, J.B. Bell, W.Y. Crutchfield and B.J. Alder Adaptive Mesh and Algorithm Refinement Using Direct Simulation Monte Carlo, J. Comput. Phys., vol. 154, pp. 134--155, 1999.
...
simulators to a systematic technique where the run-time integration of
...
this multi-scale modeling effort is described.  As given in more
detail in Cornelis et al.\,(2011a; b) a key component of G-3, also
central to its use in multi-scale modeling, is a redesign around two
workflows, one for the user and the other for developers.  The user
workflow makes explicit the linkage between modeling and experimental
results with support, for example, of experimental protocols like RTXI
(Cornelis and Coop, 2010).  Because streamlining the process of
adapting and extending GENESIS was also a key objective of the
redesign, the modular structure of G-3 now supports not only the
...
development, including the addition of simulator functionality
operating at different levels of model scale.  The G-3 architecture is
also `federated' as it extends the modular approach associated with
the development of single applications to the functional integration
of otherwise independent applications. In doing so it provides a
single collaboration framework for modeling and software development.
As described in the next section these changes have already allowed
new molecular scale modeling capacity to be rapidly incorporated into
the overall
system.\\ %They will be essential to add additional modeling
