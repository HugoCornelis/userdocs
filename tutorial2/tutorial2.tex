...
To run one of these models the model description file (NDF) must first be loaded into the GENESIS shell.  (To learn more about the NDF format see  \href{../ndf-format/ndf-format.pdf}{Introduction to the NDF File Format}.) For example, to load the multicompartment De Schutter and Bower Purkinje cell model referenced above, enter
\begin{verbatim}
    ndf_load cells/purkinje/edsjb1994.ndf
\end{verbatim}
This command results in the complete Purkinje cell model being made available to the GENESIS shell.

Note that currently it is not advisable to load more than one model at a time into the GENESIS shell. To change to a different model requires exiting the shell, restarting GENESIS, and loading the new model (see the \href{../Introduction to the GENESIS Shell}).
...
\item In the GENESIS shell the commands {\tt .} and {\tt ..} operate in the same way as when employed to traverse the Unix file system (see \\\href{../unix-linux-introduction/unix-linux-introduction.pdf}{Introduction to Unix and the Linux graphical desktop}). The {\tt ..} operator may be chained, e.g. {\tt ../../../}
