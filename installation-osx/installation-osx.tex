...
\begin{itemize}
   \item[] Download the latest version of the {\it DeveloperPackage} ({\bf Note:} Just download package, do not unpack during download as OSX will unpack the package into a directory with the package name), available from \href{http://sourceforge.net/projects/neurospaces/files/}{Sourceforge}. It is called {\it developer-release-label.tar.gz}, where {\it release-label} is the current release identifier.
