...
individual modules.  We now describe the data flows and behaviour of the CBI architecture from within the context of our ideal user workflow.\\
...
completed, these steps gnenrate a cyclic data flow
from files and databases to the {\tt Solver}s and then back.
%  Here we explain how user actions and data flows relate to
...
In Steps 1-3 of the ideal user workflow, data flows both through and between software components that conform to a CBI architecture. Data cycles between databases and files, and back-end {\tt Solver}s.  When combined with the availability of any model data from the {\tt Model\,Container}, while maintaining the integrity of the separation of concerns, it is evident that {\tt Scripting\,Libraries\,\&\,Applications} may readily be interfaced with the CBI architecture at Steps 4 and 5 of the ideal user workflow. 
%The laterality of the software component connectivity activated during Step 4 is not an issue.
(One reason for this simplification is that the back-ends of the CBI architecture, ie. all components below the dotted horizontal line in Figures ??, ??, ??, are unavailable once Step 3 is completed.)

{\tt Scripting\,Libraries\,\&\,Applications} connect to external tools such as Matlab to implement output analysis. Automated script generators are employed to generate batch simulations. Integration, at Step 4 of the ideal user workflow, with software platforms such as RTXI make direct interaction with experiment possible. Simulation output data can be combined with the model parameters and structure available in the {\tt Biology\,Model\,Container} through {\tt Scripting\,Libraries\,\&\,Applications} to implement Step 4 and 5 of our ideal user workflow. Importantly, at this stage of the workflow, as all the data of the dynamic state of the model are available through databases and files, there is no requirement to
