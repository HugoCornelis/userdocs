...
user-experience.  (For example, the Business Process Execution Language--BPEL, see http://en.wikipedia.org/wiki/Business\_Process\_Execution\_Language is a common
industry standard that was approved by the industry in 2007.)  SOAs often store control tables for
...
queries and result integration and presentation. It provides a
sharp contrast with preliminary developments in computational
...
application is in contrast to the dynamic parallel structure of a CBI
compliant simulator.  In a CBI compliant simulator the parallel
structure of the application can depend on the model structure and the
number of simulations \cite{howell00:_pgenes,
...
The {\tt Iterate} step then defines the differences in configuration
between the simulations belonging to the same simulation project,
research project, educational tutorial or scientific publication.
This step integrates the ideal user workflow into the scientific
workflow.
