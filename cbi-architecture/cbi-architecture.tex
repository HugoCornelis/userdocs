...
{\tt Solvers} that were configured for output, save results to a file.

When the simulation finishes, either by user action or when a preset
simulation period elapses, output buffers are flushed to disk.  The
independence of the {\tt Solvers} in the CBI architecture allows
solver implementations to serialize their internal state to a file for
resuming the simulation at a later date.
