...
represent\marginpar{can we give an example from a G-2 script?}.
%The result is that
...
separation of concerns.  User concerns have a direct influence on the
...
The relationships between simulator control and data modules in a federated architecture are symbolized by the horizontal arrows in Figure~\ref{fig:data-control}, whereas the relationship between high level biological concepts and their mathematical implementation are symbolized by the vertical arrows. Diagonal interactions are forbidden as they foster the mixing of functionality across different levels that ultimately leads to the creation of monolithic software applications.
...
They include: (1) Reduced\marginpar{duplicate text, it seems better to
  say that separation of concerns achieves these goals} complexity of
