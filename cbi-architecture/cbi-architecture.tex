...
Here we introduce a new class of simulator architecture, referred to as the CBI federated software architecture (CBI architecture). It is a software architecture that supports both
...
In this section we introduce the CBI architecture, a software architecture that defines a fully functioning simulator developed on the basis of the separation of concerns (described in Materials and Methods). We then report the results of a separation of concerns and introduce and describe the resulting software modules
...
set of logical relationships. In this sense the CBI architecture defines a modular framework that defines the necessary components of a neural simulator.\
...
simulations are controlled by Scripting\,Libraries\,\&Applications. The various modules and submodules located in this layer can be instantiated by a simulator on an as-needed basis. This layer supports the first three steps in the user workflow, including: (1) Model Construction, (2) Experiment Design, and (3) Run Simulation.  
...
implement a model tutorial (BOG Ref??). 
...
leading to monolithic applications (Moore, 2010).  As a consequence, the
...
software project (Brooks, 1982).  By contrast a {\it federated software design}
allows different individuals to work in parallel on the same software
application (DeRemer \& Kron, 1975).  In particular, federated
software supports the scientific enterprise by allowing individual
scientists to work on their own research projects.

% Moore JW (2010) A personal view of the early development of computational neuroscience in the USA. Frontiers in Computational Neuroscience 4: 20.
% Brooks, Frederick P., Jr. (1995). The Tar Pit, published in The Mythical Man-Month - Anniversary Edition. ISBN 0-201-83595-9.  Reading, MA: Addison-Wesley Pub. Co.
...
The {\it separation of concerns} is an important general design principle in software engineering (Dijkstra, 1976; Parnas, 1972). It aims to control the complexity of programs that are continually elaborated. In summary, it promotes the separation of different interests 
