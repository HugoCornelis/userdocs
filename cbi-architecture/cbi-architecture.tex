...
to other components of a software architecture, impact the
internal states of these components, and direct the data flows between them.  In this section we explain how user
actions and data flows relate to one another in the CBI architecture
and define the overall behaviour of an implemented software system.
...
In step 1 and 2 of the ideal user workflow users combine models and
experimental data that are stored in files and databases.  In the
third step the back-ends, such as the numerical {\tt Solvers} and the
{\tt Communication\,Infrastructure}, perform the calculations of a
simulation, then save the output back to files and databases.  When
combined these steps of the ideal user workflow imply a data flow from
files and databases to the back-ends and then back to files and
databases.
...
event that instructs the {\tt Biology\,Model\,Container} to load the
selected model from a database and store it in memory using data
structures for efficient storage and retrieval by the numerical {\tt
  Solver}.  During initial inquiry, a typical user may be interested
...
densities and reversal potentials.  A model can be queried in this way
when it is stored by the {\tt Biology\,Model\,Container}.
