...
inquiry, a typical user may be interested in derived parameters such
...
potentials.  A model can be queried in this way when it is stored by
the {\tt Biology\,Model\,Container}.

In step 2 of the ideal user workflow the user designs, as a simple
example, an experiment that applies current clamp to the neuron's
soma.  The outputs of the simulation are defined as the somatic
membrane potential and somatic transmembrane currents.
...
determine model time constants or other parameter that are relevant
...
step during this initialization instead of every for every simulation
step.  For this, the memory image of the model must be expanded in a
representation with the mathematical equations and parameters relevant
to the simulation.  In our example, the spatial layout of segments is
not required and is invisible to the {\tt Solver}.  This is different
in existing simulators, such as GENESIS 2, that do not make a
distinction between internal data structures for model represenation
and computation, and consequently often duplicate redundant data
during simulations.

In network simulations an additional step may be required that allows
