...
(Step 3), and the analysis of the dynamic state of the model (Step
4)\marginpar{new}.
...
stored in files and databases.  In the third step (Run\,Simulation)
...
combined, these steps of the ideal user workflow imply a data flow
from files and databases to the back-ends and then back to files and
databases.  Here we explain how user actions and data flows relate to
...
the {\tt Biology\,Model\,Container} to load the selected model from a
...
which is translated by the {\tt Function\,Bindings} to a sequence of
simulation-time events before the stimulation starts.  These events
...
with the {\tt Biology\,Model\,Container} examining the stored model to
...
this, the memory image of the model must first be expanded in a
representation with the mathematical equations and parameters relevant
to the simulation.  For instance, the spatial layout of segments is
not required by the {\tt Solver}.  This behaviour is different from
that of existing simulators, such as GENESIS 2, which do not make an
explicit distinction between internal data structures for model
representation and computation.  Consequently, such simulators often
generate redundant data during the initialization of a simulation.

In network simulations the {\tt Solvers} may employ the {\tt
  Connectivity\,Translator} to initialize their simulation-time
communication data structures and to connect to the {\tt
  Communication\,Infrastructure}.

When the user instructs the simulator to start a simulation, for
...
allows for better optimized implementation, but also for additional
functions such as serialization of model state to a file for later
resumption of a simulation.
...
readily be interfaced with the CBI architecture at step 4 and 5 while
