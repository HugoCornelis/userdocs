...
simulation system. It was developed on the basis of our understanding of a general scientific workflow, referred to as an {\it ideal user
...
consequence that {\it user workflow}s are often similarly degraded.
...
actions or workflow. In this section, we introduce
...
The simulator shell and the graphical user interface (GUI) each provide an interface that interprets user input such that the simulator `understands' different commands and performs the appropriate actions. Simple models can be created directly within the simulator shell by entering a sequence of commands. More complex models are available to the shell from libraries or databases external to the simulator. Shell tools can then be used to explore and check the integrity of a model. Following any necessary or desired changes, a new version of the model can be saved.
...
two submodules (illustrated in Fig.\,\ref{fig:cbi-architecture-expanded}). One is related to model data and incorporates model
...
The {\bf Command Sequence Executors} provide a physical implementation
...
the spatial layout of segments\footnote{`Segment' is a high level term employed to describe different parts of the biological model of a dendritic morphology. The equivalent low level (computational) term is `compartment'. It refers to the numerical representation of a segment.} as this is not required by the {\tt
...
simulator with new solvers is not possible because its basic
