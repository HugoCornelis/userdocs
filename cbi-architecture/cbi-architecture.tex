...
independent applications. It supports extensibility by enabling simulator expansion or enhancement without the need for
...
Modules in the top level of the CBI architecture (Fig.\,\ref{fig:cbi-architecture-simple}A) provide user accessible interfaces to simulator functionality through the GUI. Model data are controlled by the Database\,Interfaces, whereas,
...
Initially, we present two major iterative workflows (experiment and simulation),
typically employed in the conduct of scientific research. They provide
the basis of our ``philosophical workflow''. This workflow guided our development of the CBI architecture and is presented as a glossary of the technical terms employed in computer science. This is followed by a brief technical glossary of terms related to software development used in our
description of the CBI architecture. We present  an ideal ``user
workflow'' for a neural simulator. It is based on
more than 20 years of experience in the development of simulators and
their use. This architecture defines the minimal required functionality
for a neural simulator.

\subsection*{Glossary of Terms}

Many of the terms used to describe and discuss the CBI architecture
and implementation of the G-3 simulator are specialized for computer
science. The most commonly used are next described in two categories.
The first category defines terms related to the philosophical workflow
used to develop the CBI architecture.  The second category defines
incidental terms related to this process.

\subsubsection*{Monolithic Software}

A {\it monolithic software application} is comprised a set of tightly
coupled functions in the sense that modification or extension of one
function requires corresponding alterations throughout the rest of the
software code to avoid breaking existing functionality (such as
consistency and integrity).

\subsubsection*{Federated Software}

In\marginpar{move to introduction} the past neural simulators were
developed or strictly supervised by a single programmer (Moore, 2010)
leading to monolythic software code.  As a consequence the
functionality offered by computational neuroscience software and the
scientific problems it allowed to address was inherently limited in
scope by the capacity of a single person managing the required
software products.  By contrast a {\it federated software design}
allows different individuals to work in parallel on the same software
application (ref. large scale programming).  In particular, federated
...
As the last step of the philosophical workflow a {\it software
  architecture} is a collection of software modules where each modules
implements one or more concerns, but no concern is implemented by
different modules.

The {\it CBI architecture} is a framework that allows to define
software components that can be developed and tested independently.
