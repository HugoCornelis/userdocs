...
and transmembrane currents.  The {\tt Experiment\,Model\,Container}
...
functionality such as serialization of the model state and simulator output for storage in a file. This allows a simulation to be resumed at a later time.

During Step 4 the dynamic state of the model is available from the
files and databases for further analysis.

Importantly as all the data of the dynamic state of the model is
...
Steps 4 and 5 are complementary to Steps 1-3 of the ideal user
workflow.

The data cycle between databases and files, and the solvers in Steps
1-3 of the ideal user workflow results in all simulation result data
being available through these databases and files.  Once these Steps
...
As data flows in the CBI architecture the cycle between databases and
files, and the solvers in Steps 1-3 of the ideal user workflow all
model data is available through the model container.

fact that all model data is available through the model-container.
For these reasons {\tt Scripting\,Libraries\,\&\,Applications} may
readily be interfaced with the CBI architecture at Steps 4 and 5 while
maintaining the integrity of the CBI architecture by the separation of
concerns.
...
architecture for the implementation of Steps 4 and 5 (and other
Steps?), but this would limit the functional scope of the end
simulator.

All model data is available through the model container and the output
(files or databases).  The laterality of the connectivity that are
activated during Step 4 is not an issue.
