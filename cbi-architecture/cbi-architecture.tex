...
translates to a mathematical representation and a low-level software
implementation by taking many intermediate representations.  Each of
these representations is determined by the choices of the modeler.
Examples are the value of a parameter, such as the maximal
electrotonic length of a compartment, or the chosen algorithm such as
the numerical method.  Sometimes these choices have an underpinning
given by the phenomenological experiments in neuroscience.  A good
example of this is the hypothesis of Hodgkin and Huxley of the
existence of transmembrane ion channels that can be blocked by
biophysical gates
% and mathematically represent the activity of these
%gates with fitted exponentials raised to a power.
This hypothesis lead to a rich scientific discourse about the
biophysical nature of action potential propagation in an axon and
decades later the ion channels were experimentally isolated.  All
these choices taken together implement a stepwise translation of a
high-level mental model into a computer implementation.  This
translation connects mental models with their implementation, and
gives a model a richer structure and meaning than a purely
mathematical one.

A simulator must be designed to support this rich model structure and
the specific choices made during model development.  From this
viewpoint every model is intimately connected with the simulator used
to run it.  It is important to make this connection more explicit than
is currently the case \cite{Hines:2004cr}.
...
The data layering in the CBI architecture mirrors this by its gradient
from biology data to numerics.  This gradient is not unique to
computational neuroscience, so, in a broader scope, the CBI
architecture has the potential to contribute to simulator technology
...
A model is a tool useful to give a rigid structure scientific
discussions about the biology it represents.  Model collaboration is
the process of many investigators working simultaneously on the same
model and communicating about their modifications and (expected)
...
to give a trust value to the benchmark score.
...
Ultimately, the user workflow integrates a simulator with the {\it
