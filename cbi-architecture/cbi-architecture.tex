...
selected data extracted from the Biology\,Model\,Container to data
formatted in a way that is specific to the back-end.  The data
selection can omit certain properties of the biological model, i.e.
geometrical coordinates are not commonly used for the numerical
solution of an equation.

A library of functions in the Biology\,Model\,Container assists in the
translation from a continuous mathematical domain to a numerical
domain.  Examples include, the scaling of a conductance density to a
maximal conductance or the scaling of the membrane capacitance density
to the actual membrane capacitance with respect to the surface area of
a given compartment.  Superficially, this seems easy, although it is
not always the case, e.g. when spines are present in a model the
...
taken into account\cite{E:1994hc}.
...
output definitions to data structures specific to a given sequencer
implementation. This interface decouples the sequencer back-end from
the Experiment\,Model\,Container and allows them to be independently
optimized.
...
Our approach has the advantage that the mathematical and optimization internals of a
solver need not be exposed to a user.  The resulting biological and
numeric layers can further be individually optimized to allow
...
more rapidly, e.g. by achieving high cache hit ratios and parallel
...
opposed to low-level languages\,\cite{1118892}. Much more importantly however, 
the communication between modules is defined, something that can only be
...
We now introduce the user workflow and define overall design objectives for a modularized neural simulator. When combined with recently developed software tools such as simplified wrapper and interface
