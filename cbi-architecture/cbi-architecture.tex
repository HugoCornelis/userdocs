...
Finally, connecting with an Experiment\,Model\,Container and Scripting\,Libraries allows applications such as tutorials and research projects to be developed.
...
A conceptual model of a biological system translates to a mathematical
representation and a low-level software implementation by taking many
intermediate representations.  Each of these representations is the
result of a number of choices by the scientist.  Examples of these
choices are the use of algorithms such the numerical method, or the
value of parameters, such as maximal electrotonic length.  But because
of all the unknowns in neuroscience many of these choice have a
phenomenological underpinning.  Examples here are the choice of
Hodgkin and Huxley to hypothesize the existence of transmembrane ion
channels that can be blocked by biophysical gates and mathematically
represent the activity of these gates with fitted exponentials raised
to a power.  This hypothesis structured a rich scientific discourse
about the biophysical nature of action potential propagation in an
axon and decades later the ion channels were experimentally isolated.

This leads
to a stepwise process to translate a high-level model into a
computer implementation.

The richness of the mapping implemented by the intermediate layer of
the CBI architecture determines how easy it is to implement a given
user-workflow that may be specific to a research project or
educational tutorial.  Consequently, its nature, structure, and
complexities are unknown.  The first simple steps to understand this
mapping have only recently been taken.  The more elaborate
