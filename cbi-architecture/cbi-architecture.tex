...
data (numerical values) and distinguish data from control functions.
...
Here we introduce a new class of simulator architecture, the CBI {\it federated software} architecture, for convenience referred to here as the CBI architecture. As we now show, it is a {\it software architecture} that transparently supports both
...
understanding of biological systems. The\marginpar{reason} CBI architecture is designed
...
continual addition of functionality to simulators.
Ultimately\marginpar{discussion}, simulators become monolithic and it
is increasingly difficult for users and developers to maintain and
extend them, with the logical consequence that {\it user workflow}s were
...
simulation configuration\cite{cannon07:_inter}.
...
A five step outline of an {\it ideal user workflow}
...
neural simulation platform \cite{cornelis02:_tutor}.  Importantly, the workflow explicitly
...
There\marginpar{reason} are several important objectives that emerged from the separation of concerns that we used to guide the development
...
set of logical relationships. In this sense it defines a modular framework that provides the necessary parts of a neural simulator.\marginpar{definition}
...
the four quadrants of simulator functionality revealed by the separation of concerns. They 
...
Modules in the top level of the CBI architecture (Fig.\,\ref{fig:cbi-architecture-simple}A) provide user accessible interfaces to simulator functionality through a {\tt Graphical\,User\,Interface} (GUI). Model data are controlled by the {\tt Database\,Interfaces}, whereas,
...
connectivity between the low-level back-ends (described below).  For example, in a
...
experience allowed us to identify an ideal user workflow that we
employed to constrain a separation of concerns analysis that lies at
...
The CBI Architecture is based on the three-tier software architecture,
a widely used commercial client-server paradigm\,\cite{eckerson95:_three_tier_clien_server_archit}. It has the dual advantages that servers
...
Dedicated industry standards have been developed that allow routine
implementation of business logic for an optimal user-experience.
Often control tables are implemented to store meta-data describing
user workflows \cite{humby07:_progr}.  In this way control software is
replaced with software for data management and the technical
implementation becomes focused on data management.

Highly developed technologies for the expression of user experience
such as BPEL in the industry are in contrast with the preliminary
developments in computational neuroscience for the standards of
educational tutorials, research project frameworks, and declarative
modeling standards such SED-ML \cite{Kohn:2008kx}, NeuroML
\cite{Gleeson:2010cr}, NineML (http://www.nineml.org/index.shtml),
Sumatra (http://neuralensemble.org/trac/sumatra), and PyNN
\cite{davison08:_pynn}.

Control as defined for the CBI architecture is not a major aspect of
SOA implementations.  As a result SOAs are architectured using
data-centric and technical concerns such as database transactions and
data routing, logging and debugging rather than the principle concerns
of data and control we have employed here.
...
hardware servers.  However, its parallel structure is specific to a
given business application and the business logic, and it is fixed
during application design.

This is not necessarily the case in neuroscience simulators because
the CPU load of the mathematical model is often used to calculate an
ideal partitioning for each simulation making the parallel structure
...
application is in contrast to the dynamical parallel structure of a
CBI compliant simulator.  In a CBI compliant simulator the parallel
structure of the application can depend on the model structure and the
number of simulations
\cite{howell00:_pgenes}\cite{migliore06:_paral_networ_simul_neuron}.
...
Importantly the implementation of business logic as control tables has
a logical consequence that the back-ends of SOAs consist of database
engines.  Database engines embed specialized algorithms and data
...
In contrast the CBI architecture provides explicit support for control
functions throughout all the layers of a CBI compliant simulator
through its {\tt Scripting\,Libraries} and {\tt Communication}
modules.
...
the average membrane potential of a section of a neuron.
...
written.  They convert the database formats to {\it application programing interfaces} (API) calls of the
...
restricted set of extensions: extending the GENESIS 2 simulator with
new solvers was not documented because its basic architecture did not
allow it.
...
Historically\marginpar{refer again to Type 1 and Type 2 interoperability}, the emergence of monolithic neuronal software has led to
simulator interoperability in computational neuroscience being
addressed as interoperability between stand-alone
applications.  It is being proposed that the development of declarative markup languages such as NeuroML and
NineML may lead to a resolution of the simulator interoperability problem\,\cite{cannon07:_inter}.
...
%simulator.
A CBI compliant simulator can also easily support these declarative
languages by implementation of a new database interface software
component.  This database interface then maps the concepts of the
declarative languages to concepts supported by the
Biology\,Model\,Container.  At the same time it can also be used to
connect the simulator to external databases that provide support these
...
implemented using procedural paradigms\cite{ray08:_pymoos,
  davison08:_pynn}.  This approach requires the modeler to understand
some of the internals of the model, the simulator, and the
implementation of the procedural paradigm.

The CBI architecture moves these problems from a context of
communication between large monolithic applications to communication between simpler
...
In the context of the CBI architecture the solutions to both types
problems of simulator interoperability become a tangible reality.
...
The steps of the {\it Ideal User Workflow} can be categorized as
user-workflow oriented and data-oriented.  We briefly show the
relationships between the {\it Ideal User Workflow} and {\it CBI
...
these two steps, results in a single simulation that is run by the
mathematical solvers, the communication framework and the scheduler.
Each of these components needs appropriate peripheral configuration.
The methods used to analyze the results are specific to the scientific
question at hand, and can be configured in the component {\tt
  Scripting\,Libraries\,\&\,Applications}.  These configuration
definitions combined determine one deterministic run of a simulation.

The Iterate step then defines the differences in configuration between
the simulations belonging to the same simulation project, research
project, educational tutorial or scientific publication.  This step
integrates the {\it Ideal User Workflow} into the {\it Scientific
  Workflow}.
...
and a low-level software implementation.  In this process it takes
many intermediate representations that are determined by the choices
of the modeler.  Examples are the value of a parameter, such as the
maximal electrotonic length of a compartment, or the chosen algorithm
such as the numerical method.  Sometimes these choices have an
underpinning given by the phenomenological experiments in
neuroscience.  A good example of this is the hypothesis of Hodgkin and
Huxley of the existence of transmembrane ion channels that can be
blocked by biophysical gates
% and mathematically represent the activity of these
%gates with fitted exponentials raised to a power.
The sequence of these choices implement a stepwise translation of a
high-level mental model into a computer implementation.  This
translation connects mental models with their implementation directly,
and it gives a model a richer structure and meaning than a purely
mathematical one.
...
intimately connected with the simulator used to run it.  It is
...
When we refer to the choices made during translation of a mental model
to its computer implementation as model structure, and to the values
of the parameters as model quantities, then a model's history can be
defined on both model structure and model quantities.
%  Since
...
This way a modeling environment becomes a tool useful to start
formally tracking the advances in scientific understanding of a
biological system, expressed as changes made to a model.  When such a
rich model structure is supported by a computer implementation, it
gives a structure to the scientific discussions about the biology it
represents.

\subsubsection*{Computational Model Publications}
...
During this process they continuously communicate about modifications
to the model structure and model quantities, and their (expected)
consequences and interpretation.  Often it is unclear why certain
choices were made during model development because there is no way to
annotate them with the reason.  Currently this process can be tracked
formally only through scientific paper publications and with databases
of computational models, but both these communication media lack the
support to link annotations with the details of the model.
...
In general it would be useful if the details of consecutive model
versions could be annotated with user defined content.

A second class of annotations scores a version of a model for a
benchmark.  The benchmark score tells a user about strenghts,
weaknesses or deficits of a specific model version (e.g. a channel
conductance out of the physiological range -- note that both the
conductance as its valid range can be subject to experimental
evidence).
%Since the tags are
...
able to trivially check what the differences are between the two
models.  In general, it must be possible to correlate the benchmark
values with differences in (the parameters or structure of) the
physical model.  Besides differences in benchmark values and
user-contributed annotations, the parameters of the physical model,
also subtleties in model behavior when simulated can be tagged.  This
is a step closer to scientific interpretation of the model and the
biological system it represents.

This way annotations to structure and quantities of a model document
our advances in the formal scientific understanding of a given
biological system.  Ultimately, the user workflow integrates a
simulator with the {\it scientific workflow} such that any CBI
...
\end{center}
\caption{ {\bf A federated software architecture: graphical
    presentation of the different modules comprising the CBI
    architecture.  On the left side are the control modules and on the
    right side the data modules.  The top layer concerns conceptual
    data and control representations and the bottom layer concerns
    representations that are close to hardware.  The middle layer
    bridges between the two presentations.}  }
