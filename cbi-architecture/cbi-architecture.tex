...
% rather than the addition of new functionality. 
The software architecture we describe allows for both these approaches. However, it is designed to support alternative paradigms of interoperability and extensibility through the provision of well defined application programming interfaces. They allow any appropriately configured component or software application to be incorporated into a simulator.

The architecture defines independent functional modules that run stand-alone. They are arranged in logically organized layers that naturally correspond to the occurrence of high-level data (biological concepts) versus low-level data (numerical values). Independently of the technology used, the architecture can be employed by both users and developers to communicate about simulator concepts and functions. This has important advantages for facilitating multiple independent contributions to software development, including: (1) Complexity of individual simulator components is reduced compared to the complexity of a complete simulator, (2) Component can be documented in isolation, in terms of their inputs and outputs, (3) Unnecessary or obsoleted components can be easily replaced or removed, (4) Components can be tested stand-alone, and (5) Most importantly, the architecture clearly delineates the development scope of new components.
