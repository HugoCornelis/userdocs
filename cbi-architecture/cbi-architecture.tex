...
a simulation then save output to files or databases.
This implies a data flow from databases and files, that describe a model,
...
to the CBI architecture from the {\tt Scripting\,Libraries\,\&\,Applications} software component.

\paragraph{Running a Single Cell Model}

When a user wants to learn more about the electrophysiology of an
existing cell model, it must be instantiated, 
the stimulation and output protocols configured, the simulation run, and the results interpreted.

In step 1 of the ideal user workflow (Construct\,Model), the GUI is opened and a cell model is selected from a database listing of available models.
Internally, model selection is translated into an event that instructs the
{\tt Biology\,Model\,Container} to load the selected model from a database
and store it in memory using data structures for efficient
storage and retrieval by the numerical {\tt Solver}.  During initial inquiry, a typical user may be interested
in derived parameters such as the neuron's total surface area with and
without spine correction or a sophisticated GUI may present a table of
the channels employed in the model along with their conductance densities
and reversal potentials.  A model can be queried in this way when
it is stored by the {\tt Biology\,Model\,Container}.

In step 2 of the ideal user workflow (Design\,Experiment) the user designs, as a simple example, an experiment
that applies current clamp to the neuron's soma.  The outputs
of the simulation are defined as the somatic membrane potential and
somatic transmembrane currents.

Running the simulation in step 3 of the ideal user workflow (Run\,Simulation) starts
with the initialization of the {\tt Solver} data structure in memory.
For this, the stored image of the model must be expanded in a
detailed representation of only the equations and parameters relevant
to the simulation.  The spatial layout of segments is not
required and is invisible to the {\tt Solver}.  Existing simulators,
such as GENESIS 2, do not make this distinction and duplicate more
