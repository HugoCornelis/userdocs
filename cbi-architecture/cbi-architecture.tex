...
arrow connecting the {tt User\,Workflow} and {\tt Biology} modules in
...
This module contains three submodules, (1) {\tt Implementation\,Libraries} containing functions specific to a given application, and (2) Scripts that control specific series of research and tutorial simulations.  (3) The {\tt Scheduler} combines generic aspect of these scripts to instantiate the simulation.
...
of the GENESIS software platform (http://www.genesis-sim.org/). This
...
the heart of the new simulator architecture.  Importantly\marginpar{we
  want to save this sentence to end the discussion rather than start
  it} this user workflow also integrates a simulator with the {\it
...
replaced with software for data management and traditional enterprise
applications become highly data-centric.

This is in sharp contrast with the CBI architecture.  Through its
column with Scripting\,Libraries and Communication components it
provides more explicit support for control functions.
...
\subsubsection*{\bf Static Parallelization and Software Structure}
...
hardware servers.  However, any parallel structure is fixed during
design as it is specific to a given business application and the
business logic in the highest layer.  This is not necessarily the case
in neuroscience simulators because the CPU load of the mathematical
model is often used to calculate an ideal partitioning for each
simulation \cite{hines08:_neuron}.
...
module in the CBI architecture.
...
implementation of business logic.  These highly developed modules in
the industry are in sharp contrast with the preliminary developments
in computational neuroscience for the standards of educational
tutorials, research project frameworks, and declarative modeling
standards such SED-ML \cite{Kohn:2008kx}, NeuroML
...
Importantly\marginpar{this paragraph must be merged with the one
  above, it is the most important point for differences with SOA}, in
SOAs the lowest layers of database back-ends has as a logical
...
A mental model of a biological system translates to a mathematical
representation and a low-level software implementation by taking many
intermediate representations.  Each of these representations is
determined by the choices of the modeler.  Examples of these choices
are the choice of algorithms such as the numerical method, or the
value of parameters, such as maximal electrotonic length.  Many of
these choices have an underpinning given by the phenomenological
experiments in neuroscience.  Examples here are the choice of Hodgkin
and Huxley to hypothesize the existence of transmembrane ion channels
that can be blocked by biophysical gates and mathematically represent
the activity of these gates with fitted exponentials raised to a
power.  This hypothesis structured a rich scientific discourse about
the biophysical nature of action potential propagation in an axon and
decades later the ion channels were experimentally isolated.

The choices made by the modeler lead to a stepwise translation of a
high-level mental model into a computer implementation.

In this sense a model is intimately connected with the software that
is used to run it \cite{Hines:2004cr}.  It is important to make this
connection more explicit than is currently the case.  Making this
connection explicit is what we call vertical tagging in a publication
system: the system allows to tag the models with references to the
software used to run them and produce the results.
