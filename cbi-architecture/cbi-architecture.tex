...
user-workflows and gives a richer user-experience\marginpar{cite
...
with the implementation of a numerical solver. To achieve this, firstly, the source code files can be populated with the mathematical
...
module.  Following these two steps, the new solver is useful for the simulation of simple
models.  Thirdly, functions are required for communication with other
solver software components.  Finally, the interface with a
...
for run-time performance of complex queries on large datasets.
...
large databases [refs?? in this para].
...
educational tutorials, research project frameworks, and declarative modeling standards such NeuroML \cite{Gleeson:2010cr},
NineML (http://www.nineml.org/index.shtml), SED-ML \cite{Kohn:2008kx}, Sumatra (http://neuralensemble.org/trac/sumatra), and PyNN \cite{davison08:_pynn}.
...
becomes a much more tangible reality.  As the {\it API}s of the CBI
...
simulators. Nevertheless, the degree than an API of a simulator can be mapped to
work with the CBI architecture results in a measurable score of
...
of model development, (2) A database connector that enables the
...
(4) A model and result annotator that enables the automatic or manual
attachment of text to whole or part of model, and (5) An automatic
model constructor that contains software components which embed
