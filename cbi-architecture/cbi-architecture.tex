...
The lowest layer of the CBI architecture comprises numerical solvers,
so the software implementation of a CBI compliant simulator can be
started from an existing numerical solver.
...
Integrate functionality between modules.
...
The mapping implemented by the intermediate layer determines how easy
it is to implement a given user-workflow that may be specific to a
research project or educational tutorial.  The first simple steps to
understand this mapping have been taken with as a consequence that its
structure is largely unknown.  The best examples we are aware of are
...
implementation.  The data layering in the CBI architecture mirrors
...
\subsection*{CBI Architecture Extensions}
