...
As many more data flows exist than are present in reality, each actual data flow can be considered in the context of a sequence of
user actions, ie. a work flow. In the following sections, we first focus on the set of regular workflows that
use CBI simulator architecture applications, ie. where a user wants
to model a biological system. We then focus on a second set of workflows, user
extensions of the functionality of an implementation of the
architecture.

In this paper we discuss the first set of work flows by introducing an
{\it ideal user workflow} and we superficially touch upon the second
set of work flows.  All work flows described in the following sections
are technology and implementation free.  %Examples are given for
