...
actions, ie. a workflow. In this section, we introduce
the set of regular workflows that use CBI simulator architecture
applications by describing an {\it ideal user workflow}, ie. where a user wants to model a biological system. We
...
(Step 3), and analysis of the dynamic state of the model (Step
4)\marginpar{new}.
...
as they have only to deal with numerics.  Secondly, it allows for more intuitive model representation for a user.

Depending on the simulator application
domain, the implementation may focus on one of the translation functions
...
functionality above.  The behaviour of the CBI architecture is defined
by the functional and dynamic connectivity provided by these
individual modules.  We now describe the relationships of the various
modules that support this behaviour within the
...
\paragraph{Dataflows in the CBI Architecture}
...
stored in files and databases.  In Step 3 (Run\,Simulation)
the back-ends, such as the numerical {\tt Solvers} and the {\tt
...
combined, these steps of the ideal user workflow imply a cyclic data flow
from files and databases to the back-ends and then back.  Here we explain how user actions and data flows relate to
one another in the CBI architecture and define the overall behaviour
of an implemented software system.
...
storage and retrieval by the numerical {\tt Solver}.  During initial
inquiry, a user may typically be interested in derived parameters such
as the neuron's total surface area with and without spine correction
...
areas or neuron types are supported by {\tt
...
and its transmembrane currents.  The {\tt Experiment\,Model\,Container}
stores the definition of the current pulse amplitude and duration,
which is translated by {\tt Function\,Bindings} to a sequence of
simulation-time events prior to the start of a stimulation.  These events
are then executed by the {\tt Command\,Sequence\,Executor} during a
simulation.
...
phase instead of at every simulation
time step\cite{borg-graham00:_addit_effic_comput_branc_nerve_equat}.  For
this, the memory image of a model must first be expanded to a
representation that includes the mathematical equations and parameters relevant
to the given simulation.  It does not include the spatial layout of segments as this is
not required by the {\tt Solver}.  This behaviour is different from
that of existing simulators, e.g. GENESIS 2. Such simulators do not make an
explicit distinction between internal data structures for model
representation and data structures for computation.  Consequently, they often
generate redundant data during the initialization of a simulation.

In network simulations the {\tt Solver} may employ the {\tt
  Connectivity\,Translator} to initialize their simulation-time
...
were configured for output, save results to a file.

When the simulation finishes, either by user action or following a
preset simulation period, output buffers are flushed to disk.  The
independence of the {\tt Solvers} in the CBI architecture not only
allows for better optimized implementations, but also enables additional simulator
functionality such as serialization of the model state and simulator output for storage to file. This allows a simulation to be resumed at a later time.
...
In Steps 1-3 of the ideal user workflow, data flows both through and between software components that conform to a CBI architecture. Data cycles between databases and files, and back-end {\tt Solver}s.  When combined with availability of any model data from the {\tt Model\,Container}, while maintaining the integrity of the separation of concerns, it is evident that {\tt Scripting\,Libraries\,\&\,Applications} may readily be interfaced with the CBI architecture at Steps 4 and 5 of the ideal user workflow. 
%The laterality of the software component connectivity activated during Step 4 is not an issue.

One reason for this simplification is that the back-ends of the CBI architecture (ie. all components below the dotted horizontal line in Figures ??, ??, ??) are unavailable once Step 3 is completed. {\tt Scripting\,Libraries\,\&\,Applications} connect to external tools such as Matlab to implement output analysis. Automated script generators are employed to generate batch simulations and integration with software platforms such as RTXI make direct interaction with experiment possible.

Simulation output data can be combined with the model parameters and structure available in the {\tt Biology\,Model\,Container} through {\tt Scripting\,Libraries\,\&\,Applications} to implement Step 4 and 5 of our ideal user workflow. Importantly, at this stage of the workflow, as all the data of the dynamic state of the model is available through databases and files, there is no requirement to
query the software components that deal with the numerical data.
...
%Modules plugged in during Step 4 have access to all data via the model
container.
