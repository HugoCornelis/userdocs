...
Various forms of interoperability are possible and two broad types have recently been identified \cite{cannon07:_inter}.
...
simulator script languages (e.g. pyMOOSE \cite{ray08:_pymoos}) http://moose.ncbs.res.in/component/option,com\_wrapper/Itemid,86/; MUSIC \cite{Djurfeldt:2010fk}),
...
Here we introduce a new class of simulator architecture, the Computational Biology Initiative federated software architecture (for convenience referred to here as the CBI architecture). It takes its name from the Computational Biology Initiative at the University of Texas at San Antonio, where development was first initiated. It has been fully implemented as the basis of the recently reconfigured GENESIS (see \cite{Cornelis:2011fk})\marginpar{\bf Editor: Could you please complete and insert the reference for citation [28] in the References for the associated manuscript, {\it Python as a Federation Tool for GENESIS 3.0}.}. As we now show, it is a {\it software architecture} that transparently supports both
...
multi-dimensional.  These characteristcs distinguish it from the
...
simulation system that was developed on the basis of our understanding of a general scientific workflow\marginpar{\bf Hugo, this does not seem to be an appropriate reference.}\marginpar{\bf Allan, I don't understand what you mean}, an {\it ideal user
...
actions, i. e. a workflow. In this section, we introduce
the set of regular workflows that use CBI simulator architecture
applications by describing an {\it ideal user workflow} where a user wants to model a biological system. We then briefly mention a second set of workflows that comprise user extensions of the
...
(Step 3), and the analysis of this dynamic state (Step 4). We also note that this workflow does not specify any particular order for its completion. However, for any given case meaningful simulation output will only occur with completion of Steps 1--4.
...
A modeling project is established by the introduction of iterators into the user workflow. Iterators close the loop between the output of results and model construction, they include: Automated construction of simulations and batch files, static parameter searching, and active parameter searching using, for example, dynamic clamp technology. 
...
Many more possible data flows exist than there are present in reality. So each data flow must be considered in the context of a sequence of user actions, i. e. a workflow. Initially, the focus is on regular workflows using CBI simulator architecture applications, i. e. where a user wants to model a biological system. We then briefly mention other workflows that support user extensions of the functionality of an implementation of the architecture.
...
Several important objectives emerged from the separation of concern used to guide the development
...
inspiration from this partitioning.
...
simulator specific language. Instead, it interacts directly with the
{\tt Model\,Containers}\footnote{ For simplicity, where appropriate, we collectively refer to the {\tt Biology\,Model\,Container} and the {\tt Experiment\,Model\,Container} as the {\tt Model\,Containers}.} via a low-level {\it application binary interface}
...
involves separate tasks, (1) Linearization of the hierarchical
...
solvers. We now briefly expand on these tasks.
...
equations of the same type and independent of the hierarchical structure of the biological
...
This module contains three submodules, (1) {\tt Implementation\,Libraries} containing functions specific to a given application, (2) {\tt Scripting\,Libraries} that provide scripts to control a specific series of research and tutorial simulations and  (3) The {\tt Scheduler}, which combines generic aspects of these scripts to instantiate the simulation.
...
Each library has a defined focus, e.g. the investigation of a synaptic
learning rule applied to given neuron, or a library of stimulus
...
tutorial simulations.  It may connect a model to a series of stimulus
paradigms or implement `glue' functions to connect to external
libraries for other activities such as result analysis, e.g.  via the
GNU Scientific Library (http://www.gnu.org/s/gsl/), the Perl Data
...
equation (at a user settable accuracy). The generation of channel
...
experimental protocol {\it in\,computo}.  A stimulation protocol stored by the {\tt Experiment\,Model\,Container} originates a {\tt Command\,Sequence\,Executor} and the stimulus events associated with the given protocol propagate to the input port of a solver at a given time step. Note, connectivity between command executors and numerical
solvers is provided by the connectivity translation function of the {\tt Experiment\,Model\,Container}.
...
The {\tt Controller} maintains the operation of all simulator functions. It 
...
The intermediate layer comprises the bridges for function and data bindings that assist with the vertical flow of information during the translation of biological concepts into numerical equations. 
...
data structures that are specific for one type of solver.  They
...
of the {\tt Biology\,Model\,Container}.  For example, a single neuron solver only needs to access the task of biological model translation.
...
{\tt Controller} such as to start and stop a simulation. They also translate application specific control statements to events or actions for the {\tt Controller}.
...
the neuron's total surface area with and without spine correction
...
the spatial layout of segments as this is not required by the {\tt
  Solvers}.  This behavior is different from that of existing
...
back-end {\tt Solver}s.  Here we have shown this cycle for a single
...
of the GENESIS software platform. This has allowed us to identify an ideal user workflow. We employed this workflow to constrain the separation of concerns analysis that lies at
...
languages\,\cite{1118892}.  This improves support for
...
Consequently, SOA code development is highly focused on database
...
hardware servers.  However, any parallel structure is specific to a
...
architecture was not suitably modularized.
...
In this section we define the elements of our ``workflow philosophy''. It is this workflow that guides development of the CBI architecture. It is presented as a glossary of the technical terms, typically specialized for computer science, that are commonly employed in the description and discussion of the architecture. It is followed by a brief technical glossary of terms related to software development.
...
either corresponding alterations throughout or intimate knowledge of the rest
...
special technology is required to implement this type of concern
properly.
...
alignment, calling conventions which control how functions' arguments
