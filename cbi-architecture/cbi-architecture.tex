...
Software\marginpar{Allan: mush up} implementations are formal structures that map user
...
architectures can be started from the lowest layer and
...
Finally connecting with an Experiment\,Model\,Container and Scripting\,Libraries allows to build applications such as tutorials and research projects.
...
widely used client-server paradigm with the advantages that the servers
...
for run-time performance of complex queries on large datasets.  Both
specialized and general algorithms and data structures have been
...
general expectation of any query to be answered with subsecond
response time, even for very large databases\marginpar{refs?? in this
  para}.  In these architectures {\it control tables} are sometimes
used to implement user workflows\cite{humby07:_progr}.
