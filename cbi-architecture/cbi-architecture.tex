...
Software\marginpar{Allan: mush up.  This should go to results (1)?} implementations are formal structures that map user
...
The\marginpar{Allan: mush up here and following, should go to results (2)?} three-tier software
...
A CBI compliant simulator can easily support these declarative
languages by implementing a database interface software component that
maps the concepts of such a language to concepts supported by the
Biology\,Model\,Container.

As an alternative to declarative model specification, more recently
the capability to communicate simulation data at run-time between
different simulators was also implemented using procedural paradigms
but substantial knowledge of the simulator internals is
required\cite{ray08:_pymoos, davison08:_pynn}\marginpar{I think there
  is also a connection between neuron and nest, citation?}.
...
model.  Its prerequisite is that it knows to find the communication
ports of the variables computed by the mathematical solvers.
...
, while simulator extension typically occurred through
modification of existing functionality.  The CBI architecture allows
