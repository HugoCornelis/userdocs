...
\subsubsection*{Model Construction and Result Visualization}

The model construction component of the GUI allows a user to define a model in terms of
...
The model construction GUI also connects to other modeling components to provide a GUI for data base connectors and the biological and experimental model containers.

The result visualization component of the GUI provides different views of a model and supports the possible work flows between them. It can be divided into generic and application specific parts.  The generic parts are those commonly provided by external tools such as GNUplot, matlab, or xmgrace for display of the temporal evolution of variables. Examples of the application specific parts include, user specified neuron morphology visualization via morphologically related color coding of variables solved during a
...
\subsection*{Biology Model Container}

The biology model container allows a user to define a model in terms of biological properties such as spine, morphology, circuits and their connectivity. It stores a biological model which is then available for inspection by other software components. Two important functions of this container are its translation of the hierarchical biological model into an expanded numerical model for use by the solvers and translation of model connectivity to connectivity between solvers.

This container stores a conceptual representation as well as an
expanded representation of a model.  The conceptual representation
can contain algorithm names and parameters that specify how to build
the model. It can be regarded as an enumeration of biological
concepts and their relationships.  Since it supports
the conceptual representation of a model, the container can also export the
representation (e.g. store it on a file system).

By running the algorithms referenced in the conceptual representation,
an expanded representation of the model is generated.  It can be mathematical complete and can be
...
structure.  This involves the following two separate tasks.

\subsubsection*{Hierarchical Translation}

If biological components in a model are assumed to function as a single biophysical unit they can be grouped, e.g. a spine and a dendrite or an axon
  ...
  container must be able to translate it into an identifier for a
  mathematical variable.

\subsubsection*{Connectivity Translation}
...
\subsection*{Experiment Model Container}
