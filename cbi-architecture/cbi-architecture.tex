...
Actions a user takes are translated into events in a software system.
These events then impact the state of the software components, and
define the overall behaviour of the software architecture.  The events
also direct the components to pass data between one another.  In this
section we explain how user actions and data flows related to one
another in the CBI architecture.
...
Each software component is specialised in implementing one particular
function and performs the defined operations on the data it is passed.

\subsubsection*{Dataflows in the CBI Architecture}

Models and experimental data is stored in files and databases. In the
CBI architecture, the calculations performed during a simulation are
done by the numerical solvers. So there is a natural data flow from
databases and files, describing models, to the solvers.  Results from
a simulation must be analyzed.  For this analysis, function libraries
are needed, e.g. for signal analysis and binning of spikes. These
libraries are accessible to the CBI architecture from the function
library software component. So, for results analysis, there is a
natural data flow from the solvers to the function library.

\subsubsection*{Running a Single Cell Model from a Model Database}

In this workflow, a user wants to learn more about the
electro-physiology of an existing cell model.

In step 1 of the ideal user workflow, the user opens the GUI and
...
and store it in internal memory using a datastructures for efficient
storage and retrieval by other online software components.  For
example, for large models the {\tt Biology\,Model\,Container} may not
expand the model in memory, but save on memory cost by storing a
compressed image of the full model.  A user will typically be
interested in derived parameters such as the total neuron's surface
area with and without spine correction.  A sophisticated GUI may
present a table of the channels employed in the model with their
