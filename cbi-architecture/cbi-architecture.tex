...
a biological model. This step is indicated by the horizontal arrow between Scheduling and Numerics in Figure~\ref{fig:data-control} and is encompassed by the Controllers and Communication and Solvers modules illustrated in Figure~\ref{fig:cbi-architecture-simple}.\\
...
and instantiation. Figure\,\ref{fig:cbi-architecture-expanded} illustrates the various modules of the CBI
federated architecture. We now briefly describe their functionality and
relationships with reference to this figure.\\

\noindent{\bf Place Figure\,\ref{fig:cbi-architecture-expanded} about here.}\\


\subsection*{High Level Interface Layer}

Modules at the top level of the CBI architecture (northbound interfaces) provide user accessible interfaces to simulator functionality through the GUI. Model data is controlled by the modules Model\,Tools and the Biology and Experiment\,Model\,Container. Note that for convenience reference to the Model\,Container are to the Experiment\,Model\,Container. For simulation control, modules include Implementation\,Libraries and the Scheduler.
...
\subsection*{External Model Construction}

This module contains tools that enable the construction and generation of a
model visually, or otherwise.  Functionally, neuroConstruct (a software platform for developing biologically realistic 3D neural networks) and CVapp (supports editing and conversion of morphology files)
...
any tool that generates a formatted model can be connected to the biological Model\,Container.  

\subsection*{Numerical Solvers \& Sequencers}

These are low-level back-ends at the software and/or hardware level that solve numerical equations using specific algorithms. They discretize and tabulate fixed mathematical functions according to a user setable accuracy specific to the given solvers.

\subsection*{Low Level Back-Ends}
\subsubsection*{Compartmental Solvers}

The first class of back-ends (southbound interfaces) are compartmental solvers, kinetic pathway
...
\subsubsection*{Command Sequence Executors}
...
to update their internal states. This is achieved by activating Solvers and the 
...
The intermediate layer is part of the scripting API. It
...
implement a model tutorial (Ref??). The intermediate layer comprises solver and sequencer bridges, numerical translation
...
convenient way. The function of the CBI core bridges are to
...
in the simulator.  As we describe on Results, a specific implementation of a core bridge could be a
...
\marginpar{No Numerical\,Translator in
  Fig.\,\ref{fig:cbi-architecture-expanded}}
...
\marginpar{No Function\,Library in
  Fig.\,\ref{fig:cbi-architecture-expanded}}
...
\marginpar{No Database\,Connectivity in
  Fig.\,\ref{fig:cbi-architecture-expanded}}
...
\item Support divergence of model history where in each parallel model version a
  different feature is explored.
\item Support convergence of model history where the features of different model versions are integrated.
\end{itemize}
