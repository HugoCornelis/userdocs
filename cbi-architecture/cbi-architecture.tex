...
connectivity between the mathematical solvers [ref. ??].  For example, in a
...
However, during a simulation the different numerical engines are only connected by a set of communication ports.
...
physical implementation of the mathematical solvers from the biological representation of a model and the way the user sees the model.  Firstly, this greatly reduces the implementation load on the back-end solvers
...
Experiment\,Model\,Container can be enrich the simulation schedule
with actions specific for a given stimulus protocol (e.g. stimulate
the model at a particular location with a given delay in a particular
way).
...
\paragraph{\bf Scheduler:} Both the Model Container and the mathematical solver are stand-alone modules. To be useful, they must be `glued' together and activated correctly, such that they can work together in co-ordination on a single simulation. This is exactly what the scheduler does. It typically exploits the sophistication of modern  scripting languages to load software components on demand, and activate them from a configuration file. The scheduler does not have any computational load, its implementation is simple and highly configurable.
...
Controller can also implement facilitatory functions for arithmetic control and for 
...
The intermediate layer comprises the core bridges for (1) Function bindings and (2) Data bindings that assist with the vertical flow of numerical translation. It
