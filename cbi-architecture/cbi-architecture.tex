...
pulse amplitude and duration, which is translated by {\tt
  Function\,Bindings} to a sequence of simulation-time events prior to
...
relevant to the given simulation but does not include the spatial
layout of segments as this is not required by the {\tt Solver}.  This
behaviour is different from that of existing simulators, e.g. GENESIS
2. Such simulators do not make an explicit distinction between
internal data structures for model representation and data structures
for computation.  Consequently, they often generate redundant data
during the initialization of a simulation.
...
a phase of simulated calibration.
...
In Steps 1-3 of the ideal user workflow, data flows both through and
between software components that conform to a CBI architecture. Data
cycles between databases and files, and back-end {\tt Solver}s.
...
Simulation output data can be combined with the model parameters and
structure available in the {\tt Biology\,Model\,Container} through
{\tt Scripting\,Libraries\,\&\,Applications} to implement Step 5 of
our ideal user workflow, for instance to provide automated script
generators to generate batch simulations.  Importantly, at this stage
of the workflow, as all the data of the dynamic state of the model are
available through databases and files, there is no requirement to
query the software components that deal with the numerical data.  The
back-ends of the CBI architecture, below the dotted horizontal line in
Figures~\ref{fig:cbi-architecture-simple}
and~\ref{fig:cbi-architecture-expanded}, are unavailable once Step 3
is completed, preventing the implementation of diagonal interactions
and the creation of a monolythic simulator.
