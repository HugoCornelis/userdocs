...
In this section we introduce the CBI architecture. This software architecture defines a fully functioning simulator developed on the basis of a separation of concerns (described in Materials and Methods). We then describe the software modules resulting from this analysis.
...
so, federation provides transparency, heterogeneity, a high degree of function, autonomy for the underlying federated sources, extensibility, openness, and optimized performance. Federation aims to provide a unified interface to diverse applications
...
spine, morphology, and circuits and their connectivity. It also connects to other modeling components to provide a GUI for data base connectors in the module Model\,Tools and the Biological and Experimental\,Model\,Containers. This GUI can also be used to inspect or alter a model and instruct the modeling component to save the model back
...
Simulation control comprises actions such as the starting and stopping of a
...
However, during a simulation the different numerical engines are only connected by a set of communication ports. Note that because a modeler connects biological components,
...
Modules currently planned for the CBI architecture are primarily related to supporting scientific communication through electronic publication. We now briefly overview this functionality.
...
Additionally, model versions should be annotated with a tag that may
