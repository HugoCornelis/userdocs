...
the spatial layout of segments as this is not required by the {\tt
  Solver}.  This behaviour is different from that of existing
...
In network simulations the {\tt Solver} employ the {\tt
...
time and to save on total simulation time of complex models that need
a phase of calibration prior to the application of the experimental
protocol.
...
workflow, for instance to provide automated script generators to
generate batch simulations.  Importantly, at this stage of the
workflow, the back-ends of the CBI architecture, below the dotted
...
requirement to query the software components that deal with the
numerical data, which ultimately prevents the implementation of
diagonal interactions in the software and the creation of a monolythic
simulator.
