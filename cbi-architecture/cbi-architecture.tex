...
User actions are translated into events in a software system.  These
events then propagate to the components of the software system, impact
their internal state, and define the overall behaviour of the software
architecture.  The events also direct the components to pass data
between one another.  In this section we explain how user actions and
data flows relate to one another in the CBI architecture.
...
Models and experimental data is stored in files and databases. In the
CBI architecture, numerical {\tt Solvers} perform the calculations of
a simulation which then save the output back to files or databases.
This implies a data flow from databases and files, describing models,
to the {\tt Solvers} and back to files and databases.  Intermediate
results may be analyzed or aggregated by analysis libraries, e.g. for
signal analysis and binning of spikes.  These libraries are accessible
to the CBI architecture from the function library software component.


\paragraph{Running a Single Cell Model}

When a user wants to learn more about the electro-physiology of an
existing cell model, he will need to instantiate the model, configure
the stimulation and output protocol, then run the simulation and
interprete the results.

In step 1 of the ideal user workflow, this user opens the GUI and
selects a cell model from a list of available models in a database.
Internally this selection is translated to an event that instructs the
{\tt Biology\,Model\,Container} to load the model from the database
and store it in internal memory using data structures for efficient
storage and retrieval by the numerical {\tt Solver}.  For example, a
large single neuron model may be stored as a compressed image of the
full model.  During initial inquiry a typical user may be interested
in derived parameters such as the total neuron's surface area with and
without spine correction or a sophisticated GUI may present a table of
the channels employed in the model with their conductance densities
and reversal potentials.  All these queries operate on the model when
it is stored by the {\tt Biology\,Model\,Container}.

In step 2 of the ideal user workflow the user designs an experiment
that applies a simple current clamp to the neuron's soma.  The output
of the simulation is defined as the somatic membrane potential and
somatic transmembrane currents.

Running the simulation in step 3 of the ideal user workflow starts
with the initialization of the {\tt Solver} data structure in memory.
For this, the compressed image of the model must be expanded in a
detailed representation of only the equations and parameters relevant
to the simulation.  The spatial layout of segments will not be
required and is invisible to the {\tt Solver}.  Existing simulators
such as GENESIS 2, do not make this distinction and duplicated more
