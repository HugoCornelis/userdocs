...
A scheduler can provide a interface for interactive user control or
the implementation of more sophisticated user-workflows via a script.
...
representations via a simulator shell or GUI. Similarly the second
...
the physical objects used to implement them.  These interfaces
straddle the Control/Data divide and replaces the upper horizontal
...
constants such as a command voltage of a voltage clamp protocol, and
model and simulation inputs and outputs.  Other aspects of these two
steps are covered in Results\marginpar{where?}.
...
a biological model. This step is indicated by the horizontal arrow
