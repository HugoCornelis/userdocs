...
The Computational Biology Initiative federated software architecture was developed in response to the growing requirement in computational biology for simulator interoperability and extensibility. It is federated by its support of the union of independent disparate systems under a single cohesive view. It provides interoperability through its capability to communicate, execute programs, or transfer data among different independent applications such that a user requires little or no knowledge of unique characteristics. It supports extensibility through mechanisms that enable simulator expansion or enhancement without the need for major changes to system infrastructure.

Historically, simulator interoperability has relied on development of extensible markup languages such as the neuron modeling language neuroML, while simulator extension typically occurred through modification of existing functionality rather than the addition of new functionality. The Computational Biology Initiative federated software architecture allows for both these approaches. However, it is designed to support an alternative paradigm of interoperability and extensibility through the provision of well defined application programming interfaces that allow any appropriately configured component or software application to be incorporated.

The architecture defines independent functional modules, in the sense that they can be run stand-alone, arranged in logically organized layers that naturally correspond to the occurrence of high-level data (e.g. biological concepts) versus low-level data (e.g. numerical values). Independently of the technology used, the architecture can be employed by both users and developers to communicate about  global concepts and functions. This has important advantages for facilitating contributions to software development from many different people, including: (1) Complexity of individual simulator components is reduced compared to the complexity of a complete simulator, (2) Component can be documented in isolation, in terms of their inputs and outputs, (3) Unnecessary or obsoleted components can be easily replaced or removed, (4) Components can be tested stand-alone, and (5) Most importantly, the architecture clearly delineates the development scope of new components.

Here, we introduce the recently reconfigured GENESIS 3.0 neural simulator and describe how  it complies with the Computational Biology Initiative federated software architecture.

\marginpar{{\bf Hugo:} Currently 330 words, must be $\leq$\,300.}
...
One response of the computational neuroscience community to the cumbersome nature of monolithic software applications has been the development of specialized simulators capable of modeling different levels of biological detail. For example, (1) neuroConstruct assists single neuron and network construction (Gleeson et al., 2007), (2) Nest simulates large structured network systems (Diesmann \& Gewaltig, 2001), (3) HHsim provides a graphical environment for the detailed exploration of a section of excitable membrane via the Hodgkin-Huxley equation formalism (Touretzky et al., 2004), (4) COPASSI (Hoopes et al., 2006), a SBML (Hucka et al., 2004) enabled  application for simulation and analysis of sub-cellular biochemical networks, and (5) MCell which uses Monte Carlo algorithms to track the stochastic diffusion of discrete molecules (Stiles \& Bartol, 2001). 
...
Problems of incremental model extension, incomplete model specification, and reproducibility of results,  have resulted in the idea of the interoperability of neuroscience modeling software. Interoperability has been defined as ``all mechanisms that allow two or more simulators to use the same model description or to collaborate by evaluating different parts of a large neural model'' (Cannon et al., 2007), thus various forms of interoperability are possible. Two broad types have recently been identified (Cannon et al., 2007), Type\,1: Development of portable model description standards such that models built for one simulator can be run on another, i.e. the adoption of common simulation languages such as SBML (Hucka et al., 2004) or NeuroML (Goddard et al., 2001), and Type\,2: Run-time interoperability where different simulators operating on different domains interoperate at run-time either by direct coupling via simulator script languages (e.g. pyNEST--Eppler et al., 2008), indirect coupling via interpreted languages (e.g. PyNN--Davison et al., 2008), or coupling via object oriented frameworks (e.g. pyMOOSE--Ray \& Bhalla, 2008; Catacomb2--http://www.compneuro.org/catacomb/).\marginpar{HUGO--Check the accuracy of these statements}
...
The multi-dimensional complexity of neural simulation software
naturally arises from the requirements of running highly structured
and complicated models (that frequently are large), the need for realistic
{\it in-vivo} and experimental stimuli, various types of analysis and
visualization, support for different types of hardware platforms,
and software technics such as debugging and code clarity and
transparancy.
...
simulations led directly to the implementation of the GENESIS
  scripting language and a customizable
GUI\,\cite{bower98:_book_genes}.  Performance enhancements were
introduced in the context of the modeling of complicated Purkinje
cells\,\cite{deschutter94:_purkin_i, deschutter94:_purkin_ii} and
later extended to include biochemical pathway
modeling\,\cite{bhalla02:_use_kinet_genes, bhalla99:_emerg} and large
network parallel simulations\,\cite{howell00:_pgenes}. More recently, 
parameter space searching has been implemented to aid model
development\,\cite{vanier99:_compar_survey_autom_param_searc}. 
Other recent additions to  GENESIS include software releases for different
  hardware platforms and the extension of application libraries with
new functions. It has become clear, however, that the substantial
  contributions of the developer community in the past are in strong
  contrast to more recent additions by the user community that do not involve changes to the
  core of the GENESIS system.

The recent history of GENESIS shows that it has become increasingly
difficult for developers from different scientific disciplines
to contribute to simulator design and development. A significant 
reason for this was that GENESIS did not make a
separation between biological data, numerical data, and control
operations.  For example, GENESIS did not allow explicit reuse of discretized representations and
a user was required to employ a certain sequence of commands to force their
implicit reuse.  Besides both making the
simulator difficult to use and being cumbersome from an implementation
point of view, this method did not work for all circumstances, e.g.
modifying a copied discretized channel also modified the original.
Put in other words, reusing discretized representation in GENESIS
was difficult because of the 'intelligence' builtin in the simulator.

In practice, the complexity and monolithic nature of the
software results in a non-scalable software architecture.  The result
is that experimentalists are often denied the opportunity to ground
empirical observation within the formalism of computational
neuroscience.
...
architecture requires a build and release system that allows work on
one software component at a time, and at the same time automates the
integration of new code into new releases of the system. This approach supports the building of a developer community where developers are
exposed to one software component at a time and the integration of
single clearly defined software components~\cite{08:_wiki_neuros}.

\subsection*{Documentation Methods}

Because a software library or application has developers and users,
appropriate documentation must be written for both.  An additional
complicating factor is that the G-3 implementation defines many
software components.  Therefore we have developed a adequate framework
for documentation.  This framework allows users as well as developers
to step in to help to grow the CBI federated framework in a
independent fashion.
...
custom format, and we are now converting it to the widely used Doxygen
tool.
...
APIs, documented use cases clarify these technical descriptions.
...
Neurospaces supports conceptual and expanded representations of a
model, and by annotating the biological model structure with
mathematical parameters, is able to implement the translation
functionality.  At this moment, the specific Neurospaces
implementation does not allow yet to save a modified conceptual
representation back to disk.
...
where the existence of a model container would be a burden.  The
...
%\paragraph*{The monolithic approach:} The development of scientific
...
%\paragraph*{The federated approach:}
...
``separation of concerns''.  User concerns have a clear and direct
...
and guaranteed correct behaviour of the software.
...
the neurosciences. Here, we propose that 
there are two principal concerns underlying the development of
modeling software: separation between data and control and
data-layering.
...
%%modeled.
%  
%%\paragraph*{}We propose that a comprehensive simulation platform
...
implementations. The result is that many neural simulators impose mathematical
constraints on the organization of a model that interrupt the
...
The relationships between simulator control and data modules in a federated architecture are symbolized by the horizontal arrows in Figure~\ref{fig:data-control}, whereas the relationship between high level biological concepts and and their mathematical implementation are symbolized by the vertical arrows. Diagonal interactions are forbidden as they foster the mixing of functionality across different levels that ultimately lead to the creation of monolithic software applications.
...
Step three of the ideal user workflow deals with the checking, resetting, running of a simulation, and saving of the model state. This is accomplished via the Function and Data Bindings modules of the CBI architecture. These modules provide a mid-level software layer that separates the high level biological implementation from the low level numerical implementation of a model by providing translators that are capable of making intelligent decisions about mapping the given biology to the required numerics. 
...
a biological model. This step is indicated by the horizontal arrow between Scheduling and Numerics in Figure~\ref{fig:data-control} and is encompassed by the Controllers and Communication and Solvers modules in Figure~\ref{fig:cbi-architecture-simple}.
...
so, it provides transparency, heterogeneity, a high degree of function, autonomy for the underlying federated sources, extensibility, openness, and optimized performance and aims to provide a unified interface to diverse applications
...
(ref??)\marginpar{ref CBI scripting paper}.
...
In summary, the CBI federated architecture provides a template for
software development that, at its core, contains a simulator.
Additionally, the modularity of the architecture simplifies connection
to independent applications indirectly related to model construction
and instantiation. We now introduce the various components of the CBI
federated architecture and briefly describe their functionality and
relationships.\\
...
two components. One is related to model data and incorporates model
construction and visualization of simulation results, the other is
related to simulation control.

\paragraph{Model construction and result visualization:}

A model construction GUI allows a user to define a model in terms of
...
The model construction GUI also connects to other modeling components to provide a GUI for data base connectors and biological and experimental model containers.

The result visualization GUI provides different views of a model and supports the possible work flows between them. It can be divided into generic and application specific parts.  The generic parts are those commonly provided by external tools such as GNUplot, matlab, or xmgrace for display of the temporal evolution of variables. Examples of the application specific parts include, user specified neuron morphology visualization via morphologically related color coding of variables solved during a
...
for a given stimulation protocol (e.g. stimulate the model at a particular location).
...
\subsubsection*{Model Tools}

Some model tools may already incorporate a
good GUI for model construction.  The difference with those tools is
that in the CBI architecture this part of the GUI does not export the model in a simulator
specific language, but rather interacts directly with the biological and experimental model
containers.  This direct interaction creates a much tighter loop with
back-end functions.

\subsubsection*{Biology Model Container}

The biology model container ({\bf Model\,Container}) allows a user to define a model in terms of biological properties such as spine, morphology, circuits and their connectivity. It stores a biological model which is then available for inspection by other software components. Two important functions of the {\bf Model Containe}r are its translation of the hierarchical biological model into an expanded numerical model for use by the solvers and translation of model connectivity to connectivity between solvers.

The model container stores a conceptual representation as well as an
expanded representation of the model.  The conceptual representation
can contain algorithm names and parameters, that specify how to build
the model, and it can be regarded as an enumeration of biological
concepts and their relationships.  Since the model container supports
the conceptual representation of a model, it can also export this
representation (e.g. store it on a file system).

By running the algorithms referenced in the conceptual representation,
an expanded representation of the model is generated.  The expanded
representation of a model can be mathematical complete, and can be
simulated.  If a parameter is missing, e.g. the axial resistance of a
neuronal morphology, the model cannot be simulated, but is still
useful for inspection.  Note that this way of treating a model is
different from most current simulators.

By querying the modeling system in an appropriate way, summaries of
the existing model can be built (overview, table with channel
conductance parameters, etc.).

The biology model container prepares the biological model for simulations, by
translating and expanding the user defined model into a mathematical
structure.  This involves two separate tasks:

\paragraph{Hierarchical translation:}

If biological components in a model are assumed to function as a single biophysical unit they can be grouped, e.g. a spine or an axon
  hillock, or a population of calcium or potassium channels.
  For a mathematical solver however, they must be
  converted to a set of equations at the same level as other
  equations of the same type (and independent of the biological
  component). This requires that the user defined model be translated to the necessary flat stream of elements.  

  Because of the translation functionality of the biology model container, it
  must also act as an identification system. Given the identifier of a
  biological component in the expanded mathematical representation, the model
  container must be able to translate it into an identifier of a
...
This provides translation of the
  ...
  projections, each associated with its own sub-projections and connections.  However, during the
  simulation, there is only a set of communication ports, that
  connect the different numerical engines. Note that because a modeler connects biological components,
...
physical implementation of the back-end solvers from the biological representation of a model and the way the user sees the model.  This greatly reduces the implementation load on the back-end solvers
(i.e. the solvers only have to deal with the numerics).

\subsubsection*{Experiment Model Container}

The experiment model container enables the definition of an experiment in terms of actions taken on the biological model. For example, it stores a model of a stimulus paradigm and the desired output parameters and defines and stores a hierarchical sequence of stimulus-related actions (e.g. start and stop time of a current injection) and their dependencies.

\subsubsection*{Scripts \& Applications}

This component enables user-friendly work flows by providing access to applications written in high-level scripting languages that focus on specific research questions or educational tutorials. These applications are available for the definition of simulations and preparation of results for subsequent analysis.  

\subsubsection*{Implementation Libraries}
...
\subsubsection*{External Model Construction}

This component contains tools that enable the construction and generation of a
model visually, or otherwise.  Functionally, neuroConstruct and CVapp
fall into this category, as they are stand-alone applications that
are currently not easily connected with other software components.  In principle,
any tool that generates a formatted model can be connected to the model container.  
The nature of these tools is that the
interaction with the modeling container is one way, i.e.
additional actions taken by the model container are invisible for
this class of tools.

\subsection*{Back-ends}
\subsubsection*{Numerical Solvers \& Sequencers}

These are low-level back-ends at the software and/or hardware levels that solve numerical equations using specific algorithms. They discretize and tabulate fixed mathematical functions according to a user setable accuracy specific to the given solvers.

The first class of back-ends are compartmental solvers, kinetic pathway
solvers, monte carlo solvers, and in general any low-level back-end at
the numerical or hardware level.  They either implement
...
numerical solvers can easily be extended to do the necessary
...
The second class of back-ends are command sequence executors.  They provide a 
physical implementation of an
experimental protocol {\it in computo}.  A command passes a particular
value to the input port of a solver at a given time step.  Note
that connectivity between command executors and numerical
solvers is provided by the connectivity translation function of the
model container.

\subsubsection*{Discrete Representations}
...
and (2) the efficiency of the infrastructure for communicating neighboring 
...
schedule and synchronize the solvers by requesting the back-ends 
to update their internal states. This is achieved by activating the solver and the 
...
The intermediate layer, still part of the scripting API,
...
core bridge is specific for a given scripting language and makes 
the low-level SWIG interface available in a 
convenient way. The function of the CBI core bridge is to
connect the procedural descriptions of a simulation (including its
model), i.e. it facilitates decomposition into modeling versus simulation control versus simulation
runs.  More specifically, the core bridge delegates procedural
scripting statements to the correct software components
in the simulator.  As we describe below, a specific implementation of the core bridge could be a
GENESIS to G-3 software compatibility component. It is also be possible to provide a Neuron scripter that would bind to the SWIG interfaces and allow Neuron scripts to be run with a G-3 implementation of the CBI architecture.

\subsubsection*{Solver Bridge}

The solver specific bridge is a data binding that translates the
...
of a solver specific bridge can choose to bind a solver to only one of
...
The most important characteristic of the solver specific bridge is
...
\paragraph{Numerical translator}
...
The components living in this space import and/or connect the
simulator to a (set of) database(s).  They translate database specific
protocols into something that the simulator can understand: a protocol
...
view on several databases.

\subsubsection*{Model History \& Versioning}

Every model undergoes a series of changes before it is made available
to other people or published in a journal or database.  For this
reason it is important for a simulator to be able to cope with the
history of a model and it should:

\begin{itemize}
\item support model versions in a linear version history.
\item support parallel model versions where in each parallel version a
  different feature is 'tried out' (divergence of model history).
\item support model versions where features are being integrated
  (convergence of model history).
\end{itemize}
...
model contains deficits (like e.g. a channel conductance out of the
...
From a user perspective, it must be possible to check what the model
differences are between versions of the same model, that possibly
behave (slightly) differently.  Differences can be differences in
benchmark values or differences in annotations, or differences in the
...
This part of the GUI uses tools of the database connectivity box to connect
...
Examples of the user visible function of this part of the
...
(feed to the model container) and run with the simulator.

\subsubsection*{Model Collaboration}

Model collaboration is the process of many scientists working together
on the same model at the same time, and communicating about their
actions on the model and the (expected) consequences.  Model
collaboration, one of the most important aspects of science, has
always existed during scientific meetings, yet has not been
implemented over the internet.  The next generation neuronal
simulators must push the technology to make this possible.

Simultaneous collaboration technology has always been based on
sessions and session management.  The GUI for model collaboration must
be able to: show a list of collaboration servers, login and logout
with respect to a session, allow to import models from databases
into the session, and after collaborative modifications inside the
session, export it back to a database (publication).  Possibly, after
publication in the database, the databases automatically organizes to
annotate the model with benchmark scores.

The GUI allows to display the equations and biological components used
in the model, and annotate them with clear text or otherwise.
