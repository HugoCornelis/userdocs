...
extensible markup languages such as the neuron modeling language
...
extensibility through the provision of semantically\marginpar{semantic
  interface is never defined in the text} defined application
...
data (numerical values) and distinguishes data from control functions.
...
software development.  Importantly, these include: (1) Complexity of
individual simulator components is reduced\marginpar{duplicate text} compared to the complexity
...
The application of mathematical methods to modeling and quantification in neurophysiology can be traced to the Lapicque model of a neuron introduced over a century ago \cite{L:1907fk}, the empirical description of action potential generation and propagation \cite{hodgkin52:_quantitative_description}, and the application of cable theory to the modeling of dendrites \cite{Rall:1957uq}. Although, a hand cranked calculator was employed to verify the original integration of the action potential, it was not until the use of mathematical approaches based on cable theory were developed to model dendritic properties and function in the late 1950s \cite{rall59:_branc}, that digital computers became a necessary tool for modeling studies. It took a further quarter century for the interdisciplinary field that links neuroscience, cognitive science, electrical engineering, computer science, physics, and mathematics to be named and thus give birth to computational neuroscience \cite{Schwartz:1990kx}.
...
These software systems have been both highly successful and continued to grow in complexity through cycles of research project extension (see for example parallel NEURON \cite{migliore06:_paral_networ_simul_neuron} and pGENESIS \cite{Hereld:2004ly}). However, after more than twenty years of extending their functionality, usually by the direct incorporation of source code into the core of the simulator, their code structures have become so complicated that it is now increasingly difficult, if not impossible, to easily continue this process. The resulting stand-alone applications have become `monolithic' with their life cycles inevitably moving  from extension to maintenance. A significant consequence of this process is that the development of an optimized simulation can be a considerable challenge for the neuroscientist unfamiliar with the underlying mathematical and computational theory.
...
adoption of common simulation languages such as SBML \cite{Hucka-M:2004bh} or NeuroML \cite{Gleeson:2010cr}\marginpar{also nineml}, and Type\,2: Run-time
...
Here we introduce a new class of simulator architecture, the CBI federated software architecture, for convenience referred to here as the CBI architecture. As we now show, it is a software architecture that transparently supports both
...
the biological system that do not map easily to the logical principles
...
technology used by simulators exposes mathematical details and
peripheral control code that is unrelated to the biology it tries to
represent.
%The result is that
...
As a guide to help separate between these different aspects a
comprehensive user workflow can be employed by simulator users.  It
systematically organizes the user actions during model development
...
The
workflow allows a user to make distinctions between an object under
investigation, the tools used to perform the investigation, and the
operations performed during the investigation.  It further allows the
user to distinguish between the results obtained from a single
investigation and the method used to define multiple investigations in
a series.  In this way the user workflow identifies five steps in
total which are explained in more depth in Materials and Methods.

Below we introduce a meta-framework for software development that we
refer to as the CBI architecture. This framework defines a fully
functioning simulator.  It was developed on the basis of the user
workflow and an analysis method referred to as a separation of
concerns (described in Materials and Methods where definition of terms
is also given).  We then describe the software modules resulting from
this analysis.  The CBI architecture is a principle result of a
bottom-up restructuring of the source code of the GENESIS 2 neural
simulation system, described in a accompanying paper~[reference to
accompanying paper to be inserted].
...
The schema identified by the separation of concerns (see Fig.~\ref{fig:data-control}) is expanded in
...
so, federation provides transparency, heterogeneity, a high degree of function, autonomy for the underlying federated sources, extensibility, openness, and the possibility for highly optimized performance. Federation aims to provide a unified interface to diverse applications
...
architecture. We now introduce their functionality and describe their relationships within the context of the user workflow.\\
...
simulations are controlled by Scripting\,Libraries\,\&Applications. The various modules and submodules located in this layer can be instantiated by a simulator on an as-needed basis. This layer supports the user interfaces for the first three steps in the user workflow, including: (1) Construct Model, (2) Design Experiment, and (3) Run Simulation.  
...
\paragraph{\bf Model Tools:} Some model tools may already incorporate
...
Biological and Experimental\,Model\,Containers via a low-level
...
involves at least two separate tasks, (1) A
linearization\marginpar{Allan, is this ok?} of the hierarchical
...
biological component in the mathematical representation, the Biology\,Model\,Container must be able to translate it into an identifier for a mathematical variable that can be used by solver.
...
connectivity between the low-level back-ends (described below) [ref. ??].  For example, in a
...
physical implementation of the low-level back-ends from the biological representation of a model and the way the user sees the model.  Firstly, this greatly reduces the implementation complexity on the mathematical solvers
as the solvers only have to deal with the numerics.  Secondly it also allows for more intuitive model representation at the user end.

Depending\marginpar{for the discussion} on the simulator application
...
model of the biology is essentially contaminated by the structure of
...
injection) and their dependencies\marginpar{discuss separation of
...
This module contains three submodules, (1) Implementation\,Libraries containing functions specific to a given application, and (2) Scripts that control specific series of research and tutorial simulations.  (3) The generic aspect of these scripts to instantiate the simulation is called the scheduler\marginpar{Allan, ok?}.
...
learning rule.  They are typically implemented directly specific for a
set of research or tutorial simulations and connect a model to a
series of stimulus paradigms using a procedural paradigm.  There are
also `glue' functions to connect to external libraries for other
activities such as result analysis.  the GNU Scientific Library (GSL),
the Perl Data Language, and the Scientific Library for Python.
...
\paragraph{\bf Scheduler:} For stand-alone software components such as the Model\,Container and the low-level back-ends to be useful, they must be `glued' together and activated correctly, such that they can work together in co-ordination on a single simulation. This is exactly what the scheduler does. It can typically exploit the sophistication of modern scripting languages to load software components on demand, initialize and activate them from a configuration file. The scheduler does not have any computational load, its implementation is simple and highly configurable.

A scheduler can provide a script-based interface for interactive user
...
Runge-Kutta\marginpar{citation} or Crank-Nicolson\marginpar{citation}
to solve systems of equations.  Dedicated data structures can be
employed that specialize them in computational neuroscience.
...
allow significant enhancements in overall system performance.  For
...
parallel implementation\marginpar{cite multisplit neuron solver}.  For
high-level data and control, optimization involves increased support
for biological concepts and usability by employing domain specific
languages and scripting languages, as opposed to low-level
languages\,\cite{1118892}.  This allows to improve the support for
user-workflow and provide a richer user-experience\marginpar{cite
  something related to user workflow and user-experience}.

\subsection*{CBI Architecture Implementation Guide}

Software implementations are formal structures that map user
requirements to technological solutions.  Software can only meet those
requirements for which this technological mapping exists.  In other
...
architectures can only be started from the lowest layer and
constructed upwards to implement user-workflows and to meet user
requirements.

As the lowest layer of the CBI architecture comprises numerical
solvers, the implementation of a CBI compliant simulator can start
with the implementation of a numerical solver.

Firstly the source code files can be populated with the mathematical
...
module.  Now the new solver is useful for the simulation of simple
models.  Thirdly functions are required for communication with other
solver software components.  Last, the interface with a
Biology\,Model\,Container can be implemented, making the solver
available for simulation of complicated and more realistic models.

Extend an existing module.

\subsection*{Comparison to Software Industry Standards}
...
commercial implementation of a three-tier architecture where the
...
consists of database back-ends.  Database engines are highly optimized
for optimal run-time performance of complex queries on large datasets.
Both specialized and general algorithms and data structures have been
designed and investigated for the implementation of
query-acceleration.  This highly active research area, also intimately
related with research into sorting algorithms, has resulted in the
expectation of any query to be answered within a second, even for very
large databases.
...
includes functionality to automatically partition a
model\cite{cornelis07:_neuros_auto_para}\marginpar{neuron multisplit}
and to transparently distribute simulations over a cluster computer.

The business logic in enterprise applications is comparable to the
Scripting Libraries and Applications module in the CBI architecture.
The Database Interfaces are comparable to traditional ETL applications
and emerging semantic technology applications.  These highly developed
layers in the industry are in sharp contrast with the preliminary
developments in computational neuroscience for the standards of
educational tutorials, research project frameworks\marginpar{cite
  sumatra and pynn} and declarative modeling standards such NeuroML,
NineML and SED-ML\marginpar{citations}.
...
\subsection*{Simulator Interoperability and Simulator Extension}

In the context of the CBI architecture, simulator interoperability
becomes a much more physical reality.  As the APIs of this
architecture are defined, it is possible to specify the
...
work with the CBI architecture, gives the simulator a score of
...
Examples of existing or proposed model tools include: (1) A model
version history manager for the tracking and recording of the history
of model development, (2) A database connector which enables the
acquisition of a model from a database such as neuromorph.org and
converts it to a format understood by the Model\,Container, (3) A
model and result analyzer for inspection of a model stored in the
Model\,Container to extract, for example, morphological or
connectivity characteristics for quantification of model components,
(4) A model and result annotator which enables the automatic or manual
attachment of text to whole or part of model, and (5) An automatic
model constructor that contains software components that embed
algorithms for automatic model construction, for example, it enables
parameter searching and development of dendritic morphology.
...
\subsection*{From Systems of Equations to Workflows for Biological
  Research}
...
The richness of the mapping implemented by the intermediate layer
...
with as a consequence that its nature, its structure and complexities
...
simulators used in other fields, e.g. fluid dynamics.
...
history of a model. To do this a simulator must minimally support: Model versions in a linear 
...
%the internet.
Next generation neural simulators have the responsibility to target
model collaborations if they want to useful for structuring the
...
\subsubsection*{The Philosophical Workflow}
...
the smallest software component is the whole application. Monolithic
software is typically comprised of a set of tightly coupled functions
where the modification or extension of one function requires
...
In software concerns often penetrate into the different layers of an
...
As an example of the way we refer to modules and components, the GUI
...
providing an important programming paradigm. System programming languages\marginpar{define system programming languages?} are well suited for building functional software components where there is a requirement for computing speed, whereas, scripting languages are well suited for binding software components to build
...
As\marginpar{this order seems best: (1) software architecture, (2) API
  and ABI, (3) scripting languages?  Implementation technology should
  come after the philosophical workflow}. the last step of our
philosophical workflow, a {\it software architecture} defines a
framework that logically organizes a collection of modules where each
module implements one or more concerns, but no concern is identified
with more than one module. When modules are implemented as software
components and bound together with scripting languages, the components
form an application.
...
of a user workflow specific to neural simulators.
...
An {\it iterator} is an object that allows a person or system to traverse through all the elements of a collection, here the user workflow, regardless of its specific implementation.
...
This\marginpar{for discussion or introduction?}  has become manifest
in nowadays simulator scripts that are unstructured in the sense that
the biological model is mixed with periphery code for input, output
...
Below we introduce the user workflow and define overall design objectives for a modularized neural simulator. When combined with recently developed software tools such as simplified wrapper and interface
generators, the connection or interfacing of programs written in compiled computer languages such as C with a variety of interpreted scripting languages such as Python (www.python.org) and Perl (www.perl.org) is greatly simplified. 
...
They include: (1) Reduced\marginpar{duplicate text} complexity of software modules when compared to
...
A\marginpar{I still have the feeling that the user workflow is so
  fundamental to the explanation of other relevant core concepts that
  it should occur early in the paper} five step outline of a workflow
...
The validity and location of simulator output is checked prior to data analysis. Output can be analyzed either within the simulator or piped to external applications such as Matlab\marginpar{citation} for subsequent data analysis.

\subsubsection*{5. Iterate}

A modeling project is established by the introduction of iterators\marginpar{iterator used once more in a meaning different from its definition} into the user workflow. Iterators achieve this by closing the loop between the output of results and model construction. Iterators include: Automated construction of simulations and batch files, static parameter searching, and active parameter searching using, for example, dynamic clamp technology. 
...
there are two principal concerns that underly the development of
modeling software: Separation between data and control through the use
of data-layering.
...
simulation inputs and outputs.  Other aspects of these two steps are
covered in Results\marginpar{where?}.
