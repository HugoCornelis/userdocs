...
The software architecture we describe here allows for both these approaches. However, it is designed to support alternative paradigms of interoperability and extensibility through the provision of semantically defined application programming interfaces. They allow any appropriately configured component or software application to be incorporated into a simulator.
...
versus low-level data (numerical values) and distinguishes data from
control functions.

The modular nature of the architecture and its independence from a given technology facilitates communication about similar
...
In this section we introduce the CBI architecture. This software
architecture defines a fully functioning simulator developed on the
basis of a comprehensive user workflow and an analysis method referred
to as a separation of concerns (described in Materials and
Methods\marginpar{add?  Also for the definition of terms such as
  'component' and 'module' see Materials and Methods}). We then
describe the software modules resulting from this analysis.
The\marginpar{Allan, is this ok?} CBI architecture is a principle
result of a bottom-up restructuring of the source code of the GENESIS
2 neural simulation system, described in a accompanying
paper\marginpar{cite}.
...
formats employed by a computer-based model of the system.
Specifically, it is the dynamical properties of a biological system
that do not map easily to the logical principles and mathematical
constructs used by software
...
technology used by simulators expose mathematical details and
...
different aspects of the control code of the simulator software.  The
...
Biological and Experimental\,Model\,Containers via a low-level ABI.
The ABI provides a direct coupling between the implementation
technologies of the different software components, creates a much
tighter loop between GUI, Simulation\,Control and lower level back-end
functions (described below) and provides a richer interactive
user-experience.
...
Monolithic\marginpar{move to introduction?} Software and Federated
Software are the two extremes of a continuum.  The philosophical goal
of the CBI architecture is to move simulator design away from the
...
As\marginpar{it seems better to define software architecture before scripting languages?  Implementation technology should come after the philosophical workflow}. the last step of our philosophical workflow, a {\it software
