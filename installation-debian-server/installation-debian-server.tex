...
{\it This list is old and needs to be updated}.
...
\item[]{\bf texlive-humanities} is required on ubuntu based systems,
  it installs lineno.sty that is needed to compile some of the
  documents correctly.
\item[]{\bf rst2pdf} (and dependencies) is required on ubuntu based
  systems, it installs utilities to convert
  \href{http://docutils.sourceforge.net/rst.html}{restructured text}.
\item[]{\bf unrtf} (and dependencies) is required on fedora based
  systems, it installs utilities to convert
  \href{http://en.wikipedia.org/wiki/Rich_Text_Format}{rich text format}.
\end{itemize}
...
\subsubsection*{Package Building Dependencies}

If you want to use the server to build {\bf Debian} packages install the dependencies listed in the \href{../build-debian/build-debian.tex} document. To make things easier, you can just copy this command:

\begin{verbatim}
apt-get install dh-make dpkg-dev dpkg
\end{verbatim}

To build {\bf RPM} packages install the appropriate tools via the command:

\begin{verbatim}
apt-get install rpm
\end{verbatim}

To build either type of package you will need to install {\bf fakeroot}. It can be installed with apt-get:
...
To set the clock for the server from the command line install {\bf ntpdate} via apt-get as follows:
