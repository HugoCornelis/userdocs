...
According to the foregoing principles of systems theoretic analysis (see \cite{Bertalanffy:1973zr,Heylighen:2006vn}), it is clear there are numerous phenomenological approaches to the development of multi-scale models suited to the transformation of a mental model into a computational simulation.




In our approach to addressing these concerns, we have developed the concept of a cognitive workflow. It is an adaption of a previously described user workflow that organizes the sequence of necessary steps typically employed by a person in developing a computational model and employing simulation to generate data for subsequent analysis \cite{10.1371/journal.pone.0028956}. In this sense it is a depiction of a sequence of operations, declared as the work of a person or a group of persons (Belhajjame et al., 2002). Similarly, the cognitive workflow describes the process whereby an abstraction is transformed into an implementation by the conversion of a mental model into a simulation. This is made possible due to the recent reconfiguration of the GENESIS simulator. This reconfiguration




 has led to the incorporation Marr's third level (`hardware') as software functionality, to give a comprehensive multi-scale simulator. In this approach, it is the implementation of a model for simulation contained by a single software entity (here a single {\bf Biology\,Model\,Container} that incorporaties multiple solvers. The communication infrastructure enables communication at run-time between these solvers and thus across different levels of a simulation. This reduces the requirement for multiple models and simplification of intermediate modes with respect to lower levels stipulated by \cite{Sejnowski:1988fk}. 
...
Here, we describe one resolution of this situation. It was achieved with the development and implementation of a simulator as exemplified by G-3 (by virtue of its compliance with the CBI architecture) which is based on a separation of concerns that partitions data and control functions while partitioning software functionality into  `layers' that separate high level biological data from low level mathematical operations. The immediate benefit of this partitioning of the software architecture into highly modular and independent components (see \cite{10.1371/journal.pone.0028956} for details), is support multi-scale simulation.
