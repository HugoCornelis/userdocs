...
this test has a ``source'' neuron that during the simulation delivers
a spike to two ``target'' neurons.

To simulate this simple network, the simulation configuration has to
associate each neuron model in the {\bf Model\,Container} to {\bf
  Heccer}.  As of G-2, this is a manual operation.
%In G-3 multi-scale network modelling of individual neurons requires a
%{\bf Heccer} instance for each neuron.
However, these {\bf Heccer}
