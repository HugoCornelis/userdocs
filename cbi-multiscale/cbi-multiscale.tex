...
\subsubsection{Model-Container}

The {\bf Model\,Container} stores biological entities and end-user
concepts and provides a solver independent internal storage format for
models.  By `containing' the biological model, the {\bf
  Model\,Container} makes the implementation of the numerical core of
a solver independent of software implementation.

Because all the modeling components are an integrated part of a single
...
The {\bf Model\,Container} annotates the structures and concepts of a
model with biological, physical, and mathematical quantities.

Therefore in principle, the {\bf Model\,Container} is capable of
supporting the transparent construction of multiscale models and
simulations.

The {\bf Model\,Container} defines models in a purely declarative,
extensible language.
...
\subsubsection{Heccer: A Single-Neuron Solver}
...
currents\cite{segev95:_theor_found_dendr_funct}\cite{hha52}\cite{cornelis02:_tutor}.

{\bf Heccer} can be instantiated from C, Python or Perl.  It is also
possible to link {\bf Heccer} directly to Matlab. {\bf Heccer} comes
with \href{http://www.swig.org/}{\bf Swig} interface definitions, such
that linking {\bf Heccer} to any other technologies should be easy.
...
{\bf Chemesis} is a GENESIS-2 'add-on' library of objects for modeling
biochemical reactions, models of second messengers, and calcium
dynamics\cite{blackwell00:_eviden_distin_light_induc_calcium}.  As a
...
The functions currently implemented in the {\bf Chemesis-3} are:
\begin{itemize}[noitemsep,nolistsep]
\item {\bf rxnpool}: a concentration pool that interacts with
  reactions and diffuses to other pools.
\item {\bf conservepool}: mass conservation based pool, computes the
...
\item {\bf reaction}: standard forward / backward chemical reactions
  between pools of molecules.
\item {\bf diffusion}: Computes flux in molecules between two pools.
\end{itemize}
...
Both the \href{../model-container/model-container.tex}{\bf
  Model\,Container} and the mathematical solver
\href{../heccer/heccer.tex}{\bf Heccer} are stand-alone GENESIS
components. To be useful, these components must be `glued' together
and activated correctly, such that they can work together in
co-ordination on a single simulation. This is exactly what the Simple
Scheduler in Perl ({\bf SSP}) does. It is currently the standard
scheduler for GENESIS. {\bf SSP} exploits the sophistication of the
...
configurations will connect {\bf SSP} to other modeling services or
solvers.
...
GENESIS is composed of several independent software components, each
...
\href{../heccer/heccer.tex}{Heccer}, and
\href{../experiment/experiment.tex}{Experiment}, to script your
simulations via Python. However this is not desirable since all
simulations would be composed of code which contain their own control
flow, that would often times require the user to understand all of the
internals in order to expand on existing simulations. {\bf SSPy} is
designed in the same vein as the \href{../ssp/ssp.tex}{SSP (Simple
  Scheduler in Perl)} in that it encapsulates the operations for
loading and running a complete simulation while allowing for complete
control of simulation parameters and simulator options via a
declarative configuration file. It also provides an easy plugin
framework so that new modeling services, experimental protocols and
solvers can be dynamically loaded from a plugin directory, making them
immediately available without the need to change any of the core code.
For more information on the plugin framework and extending SSPy, see
the \href{../sspy-developer/sspy-developer.tex} {SSPy developer
  document}.


\subsubsection{G-Shell}

After GENESIS has completed its startup procedure you should see the
command prompt, {\tt genesis $>$}, indicating that you are now in the
GENESIS shell environment ({\bf G-Shell}) and running an interactive
GENESIS session. Note that to provide backward compatibility, the {\bf
  G-Shell} interfaces with the Script Language Interpreter (SLI) of
previous versions of GENESIS. To learn more about that functionality
see \href{../backward-compatibility/backward-compatibility.tex}{here}.
