...
\paragraph{Step 1} The first step of the addressing scheme translates NDF names to a
hierarchical namespace. It is only required during
simulation construction, not during simulation run-time. The scheme defines a symbolic syntax for the hierarchical
...
the {\bf Biology\,Model\,Container} where symbolic references are translated to
simulation wide unique tuples of the form [integer, type]. In this case (the $\mathrm{Ca}^{2+}$ pool), the
integer identifies the physical process and the type identifies the concentration level.

\paragraph{Step 2}  The second step translate the hierarchical
namespace to tuples of the form [integer, type]. This defines an interface to identify the
solver instance associated with the physical process. As it is free of symbolic references, it does not need the
...
At this step of the addressing scheme algorithm, an interface is defined that enables the communication port
