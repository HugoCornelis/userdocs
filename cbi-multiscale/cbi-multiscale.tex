...
\subsubsection{DES: Action Potential Propagation Abstraction}

The Discrete Event System ({\bf DES}) is used for the abstract
modelling of action potentials, a functionality required for the
running of network simulations.  Action potentials generated at the soma
of one neuron are translated to a discrete event that is delivered
to the post-synaptic targets of a connected neuron.

{\bf DES} can queue a single event and deliver it to many targets.  It
also associates 'secondary' data with a connection, for example, propagation delay, synaptic weight, or other data specific for the model.

From the viewpoint of the software architecture, {\bf DES} is a separate
solver that simulates action potential propagation in an efficient way.
Splitting this functionality from the rest of the simulator
facilitates customization for the modeling of sophisticated learning
rules, especially those related to STDP, diffusion, and spillover.

Internally, the discrete event system contains two subcomponents, one for event distribution that contains a connectivity matrix,
the second for event queuing.
...
\subsection{Algorithm: Addressing Scheme}

Addressing scheme: enables solvers and communication infrastructure to
find the variables of one another during simulation run-time.  Because
this happens during simulation run-time, the addressing scheme must be
highly optimized for maximum run-time performance.

The information to identify a variable (and hence its address) is
available from the model stored in the {\bf Model\,Container}.

What the {\bf Model\,Container} must do is define an addressing scheme
that allows to refer to ('address') every model component and its
simulated variables.  These references then become hooks for attaching
'experimental protocols' (output specification and stimulus
specification) and to connect the solver.  The addressing scheme is
not necessarily tied into the NDF format, because it can be more
...
consists of three steps: The first step translates NDF names to a
hierarchical namespace.  The second step translates this hierarchical
...
\begin{figure}[h!t]
  ...
  \end{center}
  \caption{ {\bf A Multi-Scale Structural Taxonomy.} }
