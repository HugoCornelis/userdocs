...
The matrix that is implicit in Figure\,\ref{fig:mental-model-simulation-path} reveals a relationship between the cognitive workflow and the three independent top-down levels of modelling previously proposed by Marr\cite{Marr:19821kx}, including:  (a) computational theory--what does the system do or what problems does it solve? (b) representation and algorithm--how does the system solve the problem? and (c) hardware implementation--how is the system physically realized? 

To explain the role of the cognitive workflow in the development of a multi-scale model, initially we introduce the reconfigured GENESIS simulator (G-3) and the primary  components that support multi-scale modelling. We then describe how to extend G-3 functionality for multi-scale modeling and give examples of interactive query and simple and multi-scale modelling scripting.
...
\caption{ {\bf Overview of a federated software architecture:}
  Graphical illustration of the primary functional modules defined for
  the CBI federated software architecture.  Control modules are given
  ...
  layer bridges between the biology and the numerics implemented in a
...
diverse workflows and low-level application programmer interfaces
...
sub-modules that comprise the CBI architecture.  We described their
functionality in \cite{10.1371/journal.pone.0028956}.  The behavior of
the CBI architecture is defined by the functional and dynamic
connectivity provided by these individual modules.  We now describe
this behavior within the context of our ideal user workflow.
...
\begin{figure}[h]
  ...
  \caption{ {\bf Detailed view of the Computational Biology Initiative
    ...
    modules of the CBI
    architecture. It shows the relationships of sub-modules
    ...
    Ultimately, the two interact to implement a single simulation.
    By design, any type of model including multi-scale models will
    exhibit this data cycle. In sharp contrast to previous versions of GENESIS and
    ...
    numerical data as needed. Additional detail in  \cite{10.1371/journal.pone.0028956}.}
...
As a result of Steps 1--3 of the user workflow, data flows both through and between software components that conform to a CBI architecture: the data cycles between databases and files, and back-end Solvers. Here we have described this cycle for a single neuron model and, as we briefly noted, it also occurs for network models. By design any type of model including multi-scale models will exhibit this data cycle.

In Steps 4 and 5, the availability of any model data from the {\bf Biology\,Model\,Container} and the functionality of {\it Scripting\,Libraries\,\&\,Applications} can connect the CBI architecture with external tools while maintaining the integrity of the separation of concerns. {\it Scripting\,Libraries\,\&\,Applications} connect to external tools such as Matlab to implement output analysis. They also allow simulation output data to be combined with the model parameters and structure available in the {\bf Biology\,Model\,Container} to implement Step 5 of the user workflow, for instance to provide automated script generators for the generation of batch simulations. Importantly, at this stage of the workflow, the back-ends of the CBI architecture (indicated below the dotted horizontal line in Figures \ref{fig:cbi-architecture-simple} and \ref{fig:cbi-architecture-expanded}) are unavailable. Once Step 3 is completed, all data generated by dynamic states of the model are available through databases and files. Consequently, there is no requirement to query software components that deal with numerical data. This ultimately prevents the creation of a monolithic simulator.
...
variables present at multiple scales as described in more detailbelow.
...
aspects of solvers and their implementation, thus better
...
The solvers currently available in G-3, starting at the highest scale:

\begin{itemize}
\item {\bf DES} discrete event queuing and distribution.
\item {\bf Heccer} single neuron solver.
\item {\bf Chemesis-3} solves simple reaction-diffusion systems.
\item {\bf Experiment} provides current injections, time-tables and
  output.
\end{itemize}
...
The {\bf Biology\,Model\,Container} stores biological entities and end-user
concepts and provides a solver independent internal storage format for
models.  By `containing' the biological model, the {\bf
Biology\,Model\,Container} makes the implementation of the numerical core of
a solver independent of software implementation.

Because all the modelling components are an integrated part of a single
model-container\cite{10.1371/journal.pone.0028956}, the existing and
...
The {\bf Biology\,Model\,Container} annotates the structures and concepts of a
model with biological, physical, and mathematical quantities.
...
The {\bf Biology\,Model\,Container} supports tokens for:
\begin{itemize}
\item Grid3D / Populations / Networks
\item VolumeConnect / Individual Connections
\item Single Cells / Morphology
\item HH-Channels / Synaptic (NMDA / AMPA) Channels
...
The model container's Neurospaces Description Format ({\bf NDF}) file format was originally designed as a file-based
user-editable declarative interface to the {\bf HSolve} method in
GENESIS-2 in 1999\cite{cornelis03:_neuros}.  In that same year its single neuron modeling capabilities were
expanded with network modeling capabilities.

The file format is indicated by the {\tt .ndf} file name extension and
integrates and extends the previous GENESIS {\tt .p} and {\tt .g}
declarative file formats.  {\bf NDF} files are purely declarative and can be stored in a model database. In an
  {\bf NDF} file, a model exists as a hierarchy of tokens, each
...
A {\bf NDF} file contains four sections. They are not necessarily
filled, but must be present in the given order.  A {\bf NDF} file has
...
The {\bf Header Section}
must starts with the {\it Unix interpreter} sequence
...
mode and keyword highlighting for Emacs and XEmacs (the Neurospaces Emacs major mode is bundled with the
  NMC package). The second declaration identifies the file type (here {\bf NDF}).

The {\bf Import Section}
...
To simplify model development, G-3 supports construction of {\bf NDF} files that define different components of a model.  These files
may be thought of as {\it library} files.  To expedite model
development, all {\bf NDF} files can be shared by being imported into
other {\bf NDF} files.

The {\bf Private Model Section}
defines models that are private to a single {\bf NDF} file.  They may depend on imported public models of
other files.

The {\bf Public Model Section} defines models are visible to the {\bf NDF} files into
...
Models declared in a
{\bf NDF} file can be `private' to that file, or `public' and
available to other {\bf NDF} files. This mechanism prevents the use
...
can depend on other {\bf NDF} files.  
...
second-order differential equation (where in computational neuroscience the right-hand side of this equation
...
\begin{equation}
  ...
  G'' + \alpha G' + \beta G = S
\end{equation}
with $\alpha = \frac{\tau_1 + \tau_2}{\tau_1 \tau_2}$, $\beta =
...
values to the two parameters, this can be expressed in the {\bf NDF} file
as:
...
Here\marginpar{makes sense?}, the {\tt DIAMETER} parameter has a constant value, the {\tt
...
ionic conductance through channels in the post-synaptic membrane.  The {\bf NDF} file format expresses the
...
token to represent the dual-exponential equation, we get the {\bf NDF}
...
Importantly, neither the {\bf NDF} file format nor
the model container
make a syntactical distinction between the variables and parameters of a
physical process.

The {\bf NDF} format supports an extensible set of tokens including {\tt
...
({\it hsolve}) of the GENESIS-2 simulator\cite{cornelis02:_tutor}.  {\bf Heccer} employs the
...
at a post-synaptic target of a connected neuron. The {\bf DES}
...
From a software architecture viewpoint, {\bf DES} is a separate
...
The functions currently implemented in {\bf Chemesis-3} include:
\begin{itemize}[noitemsep,nolistsep]
\item {\it rxnpool}--a concentration pool that interacts with
  reactions and diffuses to other pools.
\item {\it conservepool}--mass conservation based pool, computes the
  difference between the total of all molecules (a model parameter,
  rest state), and diffused molecules, divided by compartment volume.
\item {\it reaction}--standard forward / backward chemical reactions
  between pools of molecules.
\item {\it diffusion}--computes flux in molecules between two pools.
\end{itemize}
...
Both the model containers and the mathematical solver{\bf Heccer} are stand-alone G-3
...
configurations can connect {\bf SSP} to other modelling services or
solvers.

In summary {\bf SSP}:

\begin{itemize}[noitemsep,nolistsep]
\item Glues other software components together.
\item Synchronizes the activities of other components during a simulation.
...
simulations via Python. However this is not desirable as all
...
\subsubsection{G-Shell}

When running from a command line, after G-3 has completed its startup procedure, a
command prompt  ({\tt genesis\,$>$}) appears in the terminal window. This indicating that you are now in the
G-3 shell environment ({\bf G-Shell}) and running an interactive
...
component.  Python bindings for the {\bf Discrete Event System} to facilitate network modeling also exist.
...
%  compartments.}
These\marginpar{move to methods, NDF} parameters are given by their specific values (in SI units) as
...
This was previously already possible with G-2.

See tests/perl/spiker4.

The individual neurons are solved by different {\bf Heccer} instances.

The projection between them is solved by {\bf DES}.
...
\begin{verbatim}
  genesis > solverset "/network/projection => chemesis3"
\end{verbatim}
...
Below is the  G-3 representation of {\it cal1.ndf} 
...
The commands below illustrate how the {\bf G-Shell} and the {\bf G-3 Studio} (model explorer) is used
...
The {\bf G-Shell} command to run the {\it cal1.ndf} and send output to
a file:
...
For example to associate the model with name ``/purkinje'' with the solver ``heccer'', one issues the command:
\begin{verbatim}
