...
The multidimensional framework implied by Figures \ref{fig:mental-model-simulation-path} and \ref{fig:multi-scale-taxonomy} is based on the relationships between the cognitive and user workflows and the schemas outlined by Marr, Churchland, and Sejnowski.  Here, we use these relationships to develop a novel and simplifying approach to the simulation of multi-scale models. 
...
Based on these clarifications of the multi-dimensional complexity associated with modelling biological structures and functions as `simple' as a single neuron, we describe relevant features of the reconfigured GENESIS simulator (G-3). Together with several scripting examples, we show how primary components of G-3 transparently support multi-scale simulation across multiple biological levels.
