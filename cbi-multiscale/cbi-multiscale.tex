...
%space-time averages.
It is through this computational mechanism the {\bf Biology\,Model\,Container} 
...
processes and can have parameters and shared variables (described in more detail below).
...
interactions, {\it reaction.yml}: definies reactions
...
that is able to refer to or `address' the variables of every model component.  These addresses provide the `hooks' for attaching
...
\paragraph{Step 1} Translates NDF names to a
hierarchical namespace. The scheme defines a symbolic syntax for the hierarchical
addressing of model components and their parameters and variables.
The syntax allows a user to express the address of a variable, for
...
\paragraph{Step 2}  The second step is the translation of symbolic references to IDs and is implemented in
the {\bf Biology\,Model\,Container}. There, the hierarchical
namespace (created during step 1) is translated to simulation wide unique tuples of the form [integer, type]. This constructs an interface that identifies the
solver instance associated with each physical process (here the $\mathrm{Ca}^{2+}$ pool). The integer identifies the physical process and the type identifies the concentration level.

This translation is only required during
simulation construction, not during simulation run-time, thus it is free of symbolic references. Once constructed, it is independent of the
{\bf Biology\,Model\,Container}.

\paragraph{Step 3}  The third step of the addressing algorithm translates [integer, type] tuples to tuples of the form [solver instance,
internal ID]. This step is
low-level and implemented in the {\bf Solvers}.  The
advantage is that the {\bf Biology\,Model\,Container} does not have to be in memory
during simulation run-time.

At this step of the algorithm, an interface is defined that enables the communication port
of the solver that attaches to the given variables to be obtained.  This interface allows
...
low-level function that reads and writes the variable. This feature may be
useful in a parallel implementation for example.

The addressing scheme algorithm allows {\bf Solvers} to communicate through the tuples created at this step (i.e. [solver instance, internal ID]). 
This removes any requirement for
...
every cell in the network. \marginpar{Hugo: Need an example below? Prior to DES example.}  Careful G-2 script coding was required to
prevent memory exhaustion errors.  Below, we compare the G-2 addressing scheme with G-3 requirements.
