...
\begin{verbatim}
  ...
  solverset "/network/Purkinje => heccer"
...
results in a non-scalable software architecture \cite{jaeger02:_comput_neuros_realis_model_exper}.
 ...
 The CBI architecture was designed and developed in an effort to resolve the complexities associated with continual addition of functionality to simulators. As illustrated in Figure \ref{fig:cbi-architecture-simple}, the relationships between simulator control and data modules in such a software architecture are complemented by the relationship between high level biological concepts and their low level mathematical implementation.
 
 According to principles of systems theoretic analysis (see \cite{Bertalanffy:1973zr,Heylighen:2006vn}), it is clear there are numerous phenomenological approaches to the development of multi-scale models suited to the transformation of a mental model into a computational simulation. Further, as discussed in \cite{Heylighen:2006vn}, systems theory shows us that regardless of the number of scales used to conceptualize a system, it is not possible to know whether the system has been completely conceptualized. This is a consequence of changing scales which adds, deletes, and reconfigures patterns that are the content of conceptualizations. There is no way to know or learn that all patterns have been conceptualized at one or more scales, as completeness is not possible. Nevertheless, the use of multiple scales of conceptualization can be understood as a way to asymptotically approach completeness.
 
Historically, it has been proposed {Sejnowski:1988fk} that: (1) because of the the many different structural levels of organization and the fact that models rarely span more than two levels, more comprehensive understanding will require many different types of model, and (2) intermediate level models must necessarily simplify with respect to the structural properties of lower level elements, but ought to attempt to incorporate as many of the given level's functional properties as actually exist in the higher level's computational tasks. This approach has led to the development of both realistic brain models containing considerable biological detail and simplifying brain models that `abstract' both structural and functional principles, although any given model may have features of both.

Here, we describe one approach to these problems suited to computational studies. It is an approach supported by the new functionalities incorporated into the recently reconfigured GENESIS simulator by virtue of its compliance with the CBI architecture. This enhancement is obtained from the separation of data from control functions and `layering' of the simulator to separate high level biological data from low level mathematical operations. The immediate benefits of partitioning simulator software in this way (into highly modular and independent components, see \cite{10.1371/journal.pone.0028956} for details) are seen in the case of multi-scale simulation.
