...
In computational neuroscience, the term ``user workflow'' gives a sequence of steps that are typically necessary for the development and simulation of a computational model, including data generation and analysis. Such a workflow allows the object under investigation, the operations performed during the investigation, and the tools used to perform the investigation to be distinguished (for more detail see \cite{10.1371/journal.pone.0028956}). Here, we present an alternative perspective on the process  of transforming a mental model into a simulation. For convenience, we refer to it as a ``cognitive workflow'' and trace the path from abstraction to simulation in Figure\,\ref{fig:mental-model-simulation-path}. In this figure the spatial scale of the biology (the object under investigation) is located to the left, the mathematical implementation or algorithm (continuous mathematics) in the center, and the software implementation (computer science) to the right.

\vspace*{10mm}

These features of simulator use are consistent with the three independent top-down levels previously proposed by \cite{Marr:19821kx}: (a) computational theory--what does the system do or what problems does it solve?, (b) representation and algorithm--how does the system solve the problem?, and (c) hardware implementation--how is the system physically realized?
