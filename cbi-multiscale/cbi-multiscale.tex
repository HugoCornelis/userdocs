...
Extension of simulator functionality may result in the unintended creation of a monolithic software platform. This greatly increases the difficulty of integrating simulator software code to support new functionality such as efficient multi-scale modelling. In response, the research community has chosen to explore issues associated with simulator extensibility and interoperability. Analysis suggests that these issues originate in a primary focus on the biological and mathematical implementation of a specific biological model, at the expense of considering the generality of the underlying software architecture. This limits software development efforts, thus simulator functionality, and ultimately simulator extensibility and interoperability. Here, we employ axioms defined in the domain of computational neuroscience and present a logical framework that organizes our approach to multi-scale simulation. This framework underpins the modular design of the CBI federated software architecture employed to reconfigure the GENESIS simulator. It has resolved many of the problems associated with multi-scale modelling that previously existed with this simulator.  We outline our approach to development of simulator structure and function by describing simulator components essential for the implementation of multi-scale modelling. Consideration of the issues we identify greatly facilitates the development of a simulator capable of transparently supporting multi-scale biological modelling across levels ranging from the ionic and molecular to complete systems.
...
We note that the integrity of the simulator is maintained by only allowing interactions between vertically and horizontally located modules. Diagonal interactions are forbidden as they foster the mixing of functionality across different levels that inevitably leads to the existence of monolithic software applications. Ultimately, it is this partitioning of simulator functionality that lies at the core of simulator extensibility and provides the architectural foundation for transparent multi-scale simulation.
...
through the reconfiguration\marginpar{give an example} of the internal model storage format,
...
storage format to establish new declarative model tokens\marginpar{give an example} and
...
%space-time averages.
It is for these reasons that the {\bf Biology\,Model\,Container} is capable of
supporting the transparent construction of multi-scale models and
simulations.

\paragraph{Experiment\,Model\,Container}
Stores\marginpar{is not fully developed} a model of a stimulus paradigm and desired output parameters. It also defines and stores a hierarchical sequence of stimulus-related actions (e.g. start and stop time of a current injection) and their dependencies.
...
GENESIS-2 \cite{cornelis03:_neuros}\cite{beeman12:_years_comput_neuros}.  Subsequently, its single neuron modelling functionality was
...
representing a biological component.  Each component is associated with a set of physical
processes and can have parameters and shared variables. To introduce the format we now outline the general structure of an NDF file.
...
either be a constant value, point to a parameter of another symbol, or
have the value of a function.  For example:
\begin{verbatim}
PARAMETERS
...
as a numerical value may be a parameter in one model 
but a variable in another.
...
example these tokens can define a three dimensional layout of physical
processes ({\tt Grid3D}) or bind variables of selected physical
processes based on their parameterization ({\tt VolumeConnect}).
...
\cite{blackwell00:_eviden_distin_light_induc_calcium}.  The separation
of the mathematical function of the GENESIS-2 objects lead to the
implementation of {\bf Chemesis-3} as a stand-alone {\bf Solver} of
G-3.

The functions currently implemented in the core of {\bf Chemesis-3}
include: {\it rxnpool}--a concentration pool that interacts with
reactions and diffuses to other pools, {\it conservepool}--a mass
conservation based pool, it computes the difference between the total
of all molecules (a model parameter, rest state) and diffused
molecules, divided by compartment volume, {\it reaction}--provides
standard forward/backward chemical reactions between pools of
molecules, and {\it diffusion}--computes flux in molecules between two
pools.
...
of appropriate definitions for the {\bf Model\,Container} to recognize
the NDF tokens supported by {\bf Chemesis-3}.  These definitions are
given in three small text based files: {\it kinetics.yml}: supports
grouping pools of molecules and their chemical interactions, {\it
  reaction.yml}: supports the definition of reaction between pools and
{\it species.yml}: supports the definition of parameters of a chemical
species).
...
Scheduler in Perl ({\bf SSP}) and the Simple Scheduler in Python ({\bf SSPy}) do.
...
In summary, {\bf SSPy} and {\bf SSP} glue other software components together, synchronize their activity during a simulation, and are highly configurable. 
...
a command line.  After the G-3 shell environment ({\bf G-Shell}) has completed its startup procedure, a
...
The {\bf G-Shell} is
started from a system shell with the {\it genesis-g3} command  
\subsection{Interactive Query and Simulation}

Coded in Perl, the {\bf G-Shell} provides an integrated interactive
environment to the {\bf Model Container}, {\bf Heccer}, {\bf SSP},
...
A similar command exist for backward-compatible
loading of GENESIS-2 SLI scripts with {\it sli\_load}.  A {\it
  pynn\_load} to interface to
the PyNN network modeling environment \cite{davison08:_pynn} is in development.
...
between the solvers during the simulation.  The separation of the discrete event
functionality from the rest of the simulator facilitates customization
for the modeling of sophisticated learning rules, especially those
related to STDP, diffusion, and spillover\marginpar{need reference, check the G-3 grant application}.
...
simulation construction, and not during simulation run-time.  The
second step is free of symbolic references, thus does not need the
{\bf Biology\,Model\,Container}.
...
lack of distinction made by the majority of contemporary simulators between biological data, numerical data, and control operations during the design and development of a simulator. The majority of these simulators remain primarily focused on the biological and mathematical aspects of a model. As they do not respect all three of Marr's levels, i.e. the third of Marr's levels (hardware implementation) is not addressed during software design, the complexity of multi-scale simulation is greatly increased. 
...
This reconfiguration was based compliance with the CBI architecture. The CBI architecture is based on a separation of concerns that partitions data and control functions from software layering that separates high level biological data from low level mathematical operations. The immediate benefit of this partitioning of the software architecture into highly modular and independent components (see \cite{10.1371/journal.pone.0028956} for details), is transparent support of multi-scale simulation.

This approach is based on the implementation of a model for simulation being contained by a single software entity (here a single {\bf Biology\,Model\,Container} that incorporates the multiple solvers required to address the different scales of a model. A communication infrastructure enables communication at run-time between these solvers and thus across different levels of a simulation. In this paradigm, the requirement for multiple models is removed by implementation of multiple solvers. As a consequence, simplification of intermediate models with respect to lower levels as proposed by \cite{Sejnowski:1988fk} is eliminated. 
...
To this end, G-3 supports the construction of very large scale network and system
...
it aims to represent. By accounting for each of the levels proposed in \cite{Marr:19821kx} during development of a simulator, it is possible to transparently resolves many of the technical and conceptual issues currently surrounding the successful simulation of multi-scale models.
