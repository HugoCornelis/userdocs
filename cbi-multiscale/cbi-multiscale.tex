...
We further note that there is a plethora of so-called second generation simulators that remain primarily focused on the biological and mathematical aspects of multi-scale modelling. It is likely that because they did not incorporate all three of Marr's levels, the progress that might otherwise be expected from a completely configured multi-scale simulator was greatly reduced. Following the recent reconfiguration of the GENESIS simulator it is now clear that it is necessary to incorporate Marr's third level or `hardware', i.e. software functionality, to give a comprehensive multi-scale simulator.
