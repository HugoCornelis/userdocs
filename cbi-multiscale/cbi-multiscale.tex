...
The scientific understanding of problems involving the interaction between different temporal and spatial scales is very challenging, as well as being an important issue in computational neuroscience. In efforts to resolve this issue, over the last 30 years several schemas have been introduced. The earliest was proposed by Marr \cite{Marr:19821kx}, who identified three independent levels: (a) computational theory--what does the system do or what problems does it solve? (b) representation and algorithm--how does the system solve the problem? and (c) hardware implementation--how is the system physically realized? More recently, Churchland and Sejnowski \cite{Churchland:1992uq} have also proposed three levels: (1) level of analysis, which includes Marr's three levels, (2) level of processing, for example the levels of processing in the visual pathway, and (3) level of structural organization, for which they proposed seven broad levels--systems, topographic maps, layers and columns, local networks, (e) neurons, (f) synapses, and (g) molecules.

Recently, we proposed a five step ``ideal user workflow'', or user workflow, which can be employed to organize the sequence of steps typically necessary for the development and simulation of a computational model in neuroscience, including data generation and analysis. This workflow consists of the steps: construct model, design experiment, run simulation, analyze output, and iterate \cite{10.1371/journal.pone.0028956}. Such a workflow allows the object under investigation, the tools used to perform the investigation, and the operations performed during a simulation to be distinguished.

Here, we extend the workflow paradigm by introducing the organizing principle of a ``cognitive workflow''. This workflow describes the process by which a mental model is transformed from an abstraction into an implementation. One cognitive workflow that facilitates such a path is given by the top row in Figure \ref{fig:mental-model-simulation-path} (Biology\,$\rightarrow$\,Computer Simulation). In this figure, the column to the left gives the spatial scale and lists the level of structural organization of the biology (object under investigation), the column to the right lists the equivalent level of software and hardware (operations performed during the investigation), and the central column gives the mathematical implementation or algorithm (tools used to perform the investigation). The rows in each column give examples of simulator functionality appropriate for the given level. 
...
Initially, we describe features of the reconfigured GENESIS simulator (G-3), then by using scripting examples show how the primary components of the simulator transparently support multi-scale modelling from the molecular to the systems level.
...
necessary parts of a simulator.
...
data sources (see \cite{federated-2002-xyz}).
In doing so, federation provides transparency, heterogeneity, a high
...
to make major changes to the system infrastructure. Ideally, federation makes the underlying applications look like a single
system to the user. 

Enhanced application interoperability is then available
through the use of flexible high-level scripting languages to support
diverse workflows, low-level application programmer interfaces
(API)s, and application binary interfaces (ABI)s for performance (see \cite{Cornelis:2011fk}).
...
connectivity provided by these individual modules. Their behavior is now described within the context of a user workflow (for more details of this workflow see \cite{10.1371/journal.pone.0028956}).
...
In more detail, as a result of Steps 1--3 of the user workflow (construct model, design experiment, run simulation), data flows both through and between software components that conform to a CBI architecture: the data cycles between databases and files, and back-end {\bf Solvers}. Here, the cycle is described for a single neuron model. However, all scales of model will exhibit this data cycling.

Once Step 3 is completed, all data generated by dynamic states of the model are available through databases and files. Consequently, there is no requirement to query software components that deal with numerical data. This ultimately prevents the creation of a monolithic simulator.
...
unique capacity for federated and modular software development of multi-scale modelling.  Furthermore, its structure also
...
new software components to be rapidly incorporated into the simulator: 1) the modular CBI structure allows the development of new solvers independently of
...
optimization; 2) the G-3 {\it Developer\,Package} facilitates integration of
...
solvers, and finally 5) the specific development of a communication
software component to create simulation run-time interfaces between solvers.

Solvers currently available in G-3, starting at the highest scale, include: {\bf DES}, for discrete event queuing and distribution, {\bf Heccer}, a single neuron solver, {\bf Chemesis-3}, solves simple reaction-diffusion systems, and {\bf Experiment} (not to be confused with the {\bf Experimentsl\,Model\,Container}), provides current injections, time-tables, and output.
...
reliefs the implementation of the numerical core of a solver from the
...
{\em upscale} and {\em downscale} values to solvers at different
levels of scale.
%  Technically, in the case of {\bf Chemesis-3} and the
%linear cable solver, for example, these modules calculate weighted
%space-time averages.

Therefore, in principle, the {\bf Biology\,Model\,Container} is capable of
supporting the transparent construction of multi-scale models and
simulations.

For completeness we note that the {\bf Experimental\,Model\,Container} stores a model of a stimulus paradigm and desired output parameters and defines and stores a hierarchical sequence of stimulus-related actions (e.g. start and stop time of a current injection) and their dependencies.
...
user-editable declarative interface to the {\it hsolve} method in
GENESIS-2 \cite{cornelis03:_neuros}.  Subsequently, its single neuron modeling functionality was
...
The header section
typically starts with the Unix {\it interpreter} sequence
giving the system specific absolute path to the {\bf Model\,Container} stand-alone executable.
%\begin{verbatim}
%    #!/usr/local/bin/neurospacesparse
%\end{verbatim}
...
dependencies on other NDF files, e.g.
\begin{verbatim}
    IMPORT
...
%(public) components.
This supports the controlled and modular construction of a multi-scale
model as a given NDF file can depend on other NDF files.

\paragraph{Physical Processes and Model Variables in NDF}
Mathematical models can be used to describe physical processes that
underly the functional activity of biological entities by associating
...
ions in a (segment of a) dendritic tree.  The most simple of all cases
is the concentration of an ion, often $\mathrm{Ca}^{2+}$, is supposed
to follow an exponential decay according to the equation:
...
$I_{\mathrm{Ca}^{2+}}$ is a current of ions flowing into the segment.
%(here expressed in amperes).  
The parameter $B$, sometimes referred to as $\beta$, is often found by
...
In NDF the physical process of decaying calcium is represented with:
...
A parameter has either a constant value, points to a parameter of
another symbol, or has a function value.  For example:
...
'$..$' notation is a reference to the parent symbol,
...
case of a pool with calcium ions, a transmembrane calcium current may
raise the concentration level in the pool.  The current, denoted {\tt
  I} is a variable that is common to the conductance and the pool.
Such common variables that are shared between multiple physical
...
its calcium current can be bound to the pool using a {\tt BINDINGS}
clause:
\begin{verbatim}
  BINDINGS
    INPUT ../cat->I
...
Importantly, neither the NDF file format nor
the model container
make a syntactical distinction between the variables and parameters of a
physical process.
...
{\bf Heccer} also integrates exponentially decaying calcium concentrations and Nernst potentials but for 
more elaborate calcium models it must be interfaced with other solvers.
...
The Discrete Event System ({\bf DES}) is used for abstract
...
to a post-synaptic target of a connected neuron.

{\bf DES} can queue a single event and deliver it to many targets.  It
allows to associate 'secondary' data with a connection, like weight,
delay and other data specific for the model.
...
biochemical reactions, models of second messengers, and calcium
...
simulations would be composed of code which contained their own control
...
existence of a G-3 shell environment ({\bf G-Shell}), which runs an interactive
...
In this section we show how to implement two of the G-2 Chemesis tutorial examples in G-3.

The example script {\it cal1.g} creates a single compartment with interaction
between calcium and a buffer, {\it cal2.g} creates a two
compartment model with a dendrite and soma with an additional diffusion object
for diffusion between the compartments.

The G-3 representation of {\it cal1.ndf} follows: 
...
Finally, the following commands illustrate how the {\bf G-Shell} and the {\bf G-3 Studio} (model explorer) is used
to load  {\it cal1.ndf} into the {\bf Model\,Container} and explore the model:
...
The association between a given model and its solver can be configured by the user.
...
For example to associate the model named ``/purkinje'' with the solver ``heccer'', one issues the command:
\begin{verbatim}
...
See tests/perl/spiker4.

Network modelling was previously already possible with G-2. In G-3 multi-scale network modelling of 
individual neurons is also solved by different {\bf Heccer} instances. For example, 
their projections are solved by {\bf DES} where the {\bf G-Shell} syntax is:
\begin{verbatim}
  genesis > solverset "/network/target1 => heccer"
  genesis > solverset "/network/target2 => heccer"
  genesis > solverset "/network/receiver => heccer"
  genesis > solverset "/network/projection => des"
\end{verbatim}

Alternatively, in {\bf SSP} the syntax is:
\begin{verbatim}
  {
...
results in a non-scalable software architecture \cite{jaeger02:_comput_neuros_realis_model_exper}.
 ...
 The CBI architecture was designed and developed in an effort to resolve the complexities associated with continual addition of functionality to simulators. As illustrated in Figure \ref{fig:cbi-architecture-simple}, the relationships between simulator control and data modules in such a federated architecture are symbolized\marginpar{uh?? .. complemented?} by the relationship between high level biological concepts and their mathematical implementation. Diagonal interactions are forbidden as they foster the mixing of functionality across different levels that ultimately leads to the creation of monolithic software applications.
