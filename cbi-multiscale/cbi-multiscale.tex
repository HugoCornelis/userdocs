...
Here, we describe one resolution of this situation. It is relies on the introduction of divergence at the level of the cognitive workflow. Ultimately, this allows new functionalities to be incorporated into the development and implementation of a simulator as exemplified by G-3 (by virtue of its compliance with the CBI architecture). This divergence is obtained from a separation of concerns that partitions data and control functions while partitioning software functionality into  `layers' that separate high level biological data from low level mathematical operations. The immediate benefit of this partitioning of the software architecture into highly modular and independent components (see \cite{10.1371/journal.pone.0028956} for details), is to introduce principled divergence into the cognitive workflow. The primary advantage, is the appropriate configuration of a simulator to support multi-scale simulation.
