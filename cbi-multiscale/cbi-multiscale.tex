...
In computational neuroscience, the term ``user workflow'' can be used to describe a sequence of steps typically necessary for the development and simulation of a computational model, including data generation and analysis. Such a workflow allows the object under investigation, the operations performed during the investigation, and the tools used to perform the investigation to be distinguished (for more detail see \cite{10.1371/journal.pone.0028956}).

Here, we extend the workflow paradigm by introducing a ``cognitive workflow'' that describes the process by which a mental model can be transformed from an abstraction into an implementation. One cognitive workflow that facilitates such a path is illustrated in Figure\,\ref{fig:mental-model-simulation-path}. In this figure, the columns give the spatial scale of the biology (the object under investigation) to the left, the software implementation (computer science) to the right, and the mathematical implementation or algorithm (continuous mathematics) in the center. The rows in each column give simulator functionality appropriate for the given structural level of the biology. 
...
We also describe a relationship between the cognitive workflow and three independent top-down levels of modelling previously proposed by Marr\cite{Marr:19821kx}. These include:  (a) computational theory--what does the system do or what problems does it solve? (b) representation and algorithm--how does the system solve the problem? and (c) hardware implementation--how is the system physically realized? Finally, we describe the implications of this workflow for multi-scale modelling. 
...
(API)s and application binary interfaces (ABI)s for performance.
...
GENESIS version 3.0 or G-3.  The structure of G-3 provides a new and
unique capacity for federated and modular software development in
support of multi-scale modeling.  Furthermore, its structure also
...
while during the simulation run-time this is accomplished through
...
Following the CBI-architecture several explicit features of G-3 allow
the rapid incorporation of new software components: 1) the modular
separation of the development of the core of new solvers from that of
other software components allows full focus on the mathematical
aspects of the solver and their implementation, and better
optimization; 2) the G-3 Developer package facilitates integration of
new solvers into the build system of G-3 enabling immediate regression
tests; 3) easy extension of the configuration of the internal model
storage format to establish new declarative model tokens and
parameters recognized by the new solvers; 4) easy construction of an
interface between the internal storage format and the core of the new
solvers and finally 5) the specific development of a communication
software component to create simulation run-time interfaces between
the solvers.
