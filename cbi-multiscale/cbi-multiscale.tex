...
solver instance associated with each physical process (here the $\mathrm{Ca}^{2+}$ pool). The integer identifies the physical process and the type identifies the variable containing the concentration level.

This translation is only required during
simulation construction, not during simulation run-time. Once constructed, the tuples are independent of the
