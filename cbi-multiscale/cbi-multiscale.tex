...
It is for these reasons that it was possible to consider realistic and simplifying brain models to be the end points of a continuum, and that any given model may have features of both. Further, to predict, that future brain models will be intermediate types that combine the advantages of both realistic and simplifying models\cite{Sejnowski:1988fk}.
...
This view has been changed with the formulation of systems theory (see \cite{Bertalanffy:1973zr}) that, in contrast with the elements of Newtonian ontology (matter, the absolute space and time in which that matter moves, and the forces or natural laws that govern movement), considers living systems to be intrinsically open. They have to interact with their environment, and absorb and release matter and energy to stay alive. The reason for the predictive success of Newtonian models was that they only considered closed systems, whereas, open systems depend on an environment much larger and more complex than the system itself, so that its effect can never be truly controlled or predicted\cite{Heylighen:2006vn}. Regularity or organization is not a given, but emerges dynamically out of a tangle of conflicting forces and random fluctuations, a process aptly summarized as ``order out of chaos''\cite{Prigogine:1984ve}.

Recent developments in the multi-scale definition of a system define it as a collection of properties and relationships that exhibit an essential duality\cite{Heylighen:2006vn}. It is both a whole and a part of a more expansive whole or `holon'\cite{Koestler:1990ly}, i.e. there is a cohesion to its parts that allows it to be treated as a whole; and that whole itself is part of the system's environment. There are at least two fundamental reasons why is is not possible to conceptualize a system defined in this way: (1) It is only possible to conceptualize at specific scales, not conceptualize with combinations of conceptualizations at multiple scales, and (2) It is not possible to conceptualize properties separately when they are parts of a system as they are always `of' something. At any scale of conceptualization, properties are always aggregated at least as simple objects. A system at any scale of conceptualization is an approximation or loose congruence of the system. A scale specific conceptualization of a system is partial if there are patterns pertinent to an observer's interest in a system that are only available at other scales (where patterns are simple objects, relationships, and their aggregations). This is not an `abstraction' of the system as this means leaving things out with the understanding that what is left out can always be reinserted. What is ``left out'' of a scale specific conceptualization of a system are patterns that might be available at other scales. Such patterns cannot be returned to a scale specific approximation of a system, regardless of resolution or field of view. The `missing' content has no place in the conceptualization as it was never available to begin with.
...
The third definition is the one employed here: (3) Software definition: Based on the CBI architecture: the application
