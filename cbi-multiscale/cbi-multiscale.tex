...
G-2\cite{cornelis02:_tutor}.  

To simulate networks of cells the user must manually initialize the
numerical solver of G-2 initialized for every cell in the network.
Careful G-2 script coding is required to prevent memory exhaustion
errors.


In G-3 multi-scale network modelling of individual neurons requiresa
{\bf Heccer} instance for each neuron.  However, these {\bf Heccer}
instances can share internal data structures as requried, thereby
reducing memory consumption.

The {\bf SSP} software
component defines several software regression tests with network
models.  One of the network models used in this test has a ``source''
neuron that during the simulation delivers a spike to two ``target''
neurons.
