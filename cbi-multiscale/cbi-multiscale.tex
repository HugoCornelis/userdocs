...
Extension of simulator functionality may frequently result in the unintentional creation of monolithic software platforms. Typically, this greatly increases the difficulty of extending simulator software code to support new functionality such as efficient multi-scale modelling. As one alternative, the modular design of the CBI federated software architecture has resolved this problem for the recently reconfigured GENESIS simulator. We present an analysis of core axioms in computational neuroscience and conclude that the difficulties of simulator extensibility faced by many software developers originates in their primary focus on the biology and mathematical implementation of a model, at the expense of considering the underlying software architecture. This limits software development efforts, thus simulator functionality, and ultimately simulator extensibility. We outline our approach to development of simulator structure and function by describing essential components of the GENESIS simulator that conform to the layers of the CBI architecture. Careful consideration of the issues we identify greatly facilitates the development of a simulator capable of transparently supporting multi-scale biological modelling across biological levels from the ionic and molecular to complete systems.
...
Scientific understanding of problems involving the interaction between different temporal and spatial scales is very challenging and is an area of active research in computational neuroscience. For example, over the last 30 years several schemas have been introduced. The earliest was a proposal by Marr \cite{Marr:19821kx} who identified three independent approaches to understand visual processing: (a) computational theory--what does the system do or what problems does it solve? (b) representation and algorithm--how does the system solve the problem? and (c) hardware implementation--how is the system physically realized? More recently, Churchland and Sejnowski \cite{Churchland:1992uq} have also proposed three levels: (1) level of analysis, which includes Marr's three levels, (2) level of processing, for example the levels of processing in the visual pathway, and (3) level of structural organization, for which they proposed seven general levels--systems, topographic maps, layers and columns, local networks, (e) neurons, (f) synapses, and (g) molecules.

Recently, we proposed a five step ``ideal user workflow'', or user workflow. This workflow organizes the sequence of steps typically employed for the development and simulation of a computational model, including data generation and analysis. It consists of five steps: construct model, design experiment, run simulation, analyze output, and iterate \cite{10.1371/journal.pone.0028956}. Such a workflow allows the object under investigation, the tools used to perform the investigation, and the operations performed during a simulation to be distinguished. For clarification, in the biological domain we refer to `multi-level' (level), in the modelling domain we refer to `multi-scale' (scale), and in the software domain we refer to `multi-layer' (layer).

Here, we extend our workflow paradigm by introducing the organizing principle of a ``cognitive workflow''. This workflow describes the process whereby an abstraction is transformed into an implementation by the development of a mental model into a simulation. One cognitive workflow that facilitates such a path is given by the top row in Figure \ref{fig:mental-model-simulation-path} (Biology\,$\rightarrow$\,Computer Simulation). In this figure, the column to the left gives the spatial scale and lists the level of biological organization (object under investigation), the column to the right lists the equivalent layers of software and hardware (operations performed during the investigation), and the central column gives the scale of the mathematical implementation or algorithm (tools used to perform the investigation). The rows in each column give examples of the functionality appropriate for the given level.
...
Each step in the cognitive workflow can be expanded into a matrix where columns span the range of desired detail at each given biological level. As an example, Figure \ref{fig:multi-scale-taxonomy} expands the Biology step of the cognitive workflow to give a matrix that describes a taxonomy of possible models. Each row in this matrix  illustrates a set of models that spans the range of model detail at a given biological level.
...
The matrix implicit in Figure \ref{fig:mental-model-simulation-path} reveals a relationship between the cognitive and user workflows and the schema outlined by Marr.  Here, we use these relationships to develop a novel and simplifying approach to the simulation of multi-scale models. 

If it is accepted that: (1) a scale specific approximation can be abstracted at each level of a system and disjointly partitioned from its environment, and (2) the biological structures listed in Figure \ref{fig:mental-model-simulation-path} are plausible, then it is apparent that a realistic neuron model comprised of at least a soma and dendrites and that includes channels and synapses is a multi-level model simulated at multiple scales. 

Based on this clarification of the multi-dimensional complexity associated with modelling even the simplest biological structures and functions, we describe relevant features of the reconfigured GENESIS simulator (G-3). This is followed by several scripting examples that show how the primary components of the simulator transparently support multi-scale modelling from the molecular to the systems level.
...
form the building blocks of the CBI architecture.  It retains
the four quadrants of simulator functionality identified by the
separation of concerns, including the notions of low-level data for
...
data sources (see \cite{federated-2002-xyz}). Ideally, it makes the underlying applications look like a single
...
performance. Here, extensibility is defined as a system design
...
to make major changes to the system infrastructure. 
...
functionality has previously been described in \cite{10.1371/journal.pone.0028956}.  The behavior of
...
\begin{figure}[h!t]
  ...
  \caption{ {\bf Detailed view of the Computational Biology Initiative
    ...
    implementation of new solvers and other run-time software
...
Once Step 3 is completed, all data generated by dynamic states of the model are available through databases and files. Consequently, there is no requirement to query software components that deal with numerical data. This is a further functional barrier that prevents the creation of a monolithic simulator.

In Step 4 of the user workflow (output), the availability of any model data from the {\bf Biology\,Model\,Container} and the functionality of {\it Scripting\,Libraries\,\&\,Applications} can be used to connect the CBI architecture to external tools, for example Matlab (to implement output analysis), while maintaining the integrity of the separation of concerns. At Step 5 (iterate), simulation output data and model parameters and structure available in the model containers can be combined, for example, to provide automated script generators for the creation of batch simulations. Importantly, at this stage of the workflow, the back-ends of the CBI architecture (indicated below the dotted horizontal line in Figures \ref{fig:cbi-architecture-simple} and \ref{fig:cbi-architecture-expanded}) are unavailable. 
...
The software architecture developed for G-3 (http://genesis-sim.org) supports a new and
...
supports the direct interaction or embedding of models at different
...
while during simulation run-time it is accomplished through
...
new software components to be rapidly incorporated into the simulator: 1) modularity of the CBI architecture supports the development of new solvers independently of
other software components. This encourages full focus on the mathematical
aspects of solvers and their implementation and leads to better
optimization; 2) the {\it Developer\,Package} facilitates integration of
new solvers into the build system of G-3 enabling immediate regression
...
parameters recognized by new solvers; 4) easy construction of an
interface between the internal data storage format and the core of the new
solvers, and finally 5) a communication
software component that creates simulation run-time interfaces between solvers.

Solvers currently available in G-3, starting at the highest level, include: {\bf DES}, a discrete event queuing and distribution infrastructure, {\bf Heccer}, a single neuron solver, {\bf Chemesis-3}, a simple reaction-diffusion system solver, and {\bf Experiment}, provides current injections, time-tables, and output (not to be confused with the {\bf Experiment\,Model\,Container}).
...
{\bf Biology\,Model\,Container} stores biological entities and
...
new G-3 solvers can be integrated to simulate different aspects of the
...
{\em upscale} and {\em downscale} values to solvers operating at different
levels of scale.
...
For completeness, we note that the {\bf Experiment\,Model\,Container} stores a model of a stimulus paradigm and desired output parameters and defines and stores a hierarchical sequence of stimulus-related actions (e.g. start and stop time of a current injection) and their dependencies.
...
may be thought of as {\it library} files.  To expedite model
...
where $I_{\mathrm{Ca}^{2+}}$ is an ionic current flowing into the segment
%(here expressed in amperes).  
and $B$ is the decay constant (sometimes referred to as $\beta$), often found by
...
process.  For example, {\tt
  LENGTH} is neither defined as a parameter with a fixed value nor as a solved variable.
...
processes, must be declared using the {\tt BINDABLES} keyword.
\begin{verbatim}
  BINDABLES
    INPUT I
  END BINDABLES
...
it can be bound to the pool by using a {\tt BINDINGS}
clause:
\begin{verbatim}
  BINDINGS
    INPUT ../cat->I
...
The NDF file format supports an extensible set of tokens including {\tt
  ...
  PROJECTION}, {\tt NETWORK}, amongst others, that allow the construction of
models ranging from subcellular dynamics up to the network level.


\subsubsection{A Single-Neuron Solver: Heccer}

{\bf Heccer} is a G-3 reimplementation of the compartmental solver
...
more elaborate calcium models must be interfaced with other solvers.
...
Both the model containers and the mathematical solvers are stand-alone G-3
...
simulations via Python (see below). However, this is not desirable as all
simulations would be composed of code which determined their own control
...
loading and running a complete simulation while giving complete
...
solvers can be dynamically loaded from a plugin directory. This extends simulator functionality without the need to change any core code.
...
existence of a G-3 shell environment ({\bf G-Shell}) is running an interactive
...
Integration of the {\bf G-Shell} with the {\bf Model Container} enables
real-time analysis of the quantitative and structural aspects of a
neuronal morphology.  The library of model components
installed with the {\bf Model Container} provides, for example, the definition of a
...
is a simple front-end to the {\bf Model\,Container}.
...
Chemesis tutorial example in G-3. It creates a single compartment that
...
The following commands illustrate how the {\bf G-Shell} and the {\bf G-3 Studio} (model explorer) may be employed
to load  {\it cal1.ndf} into the {\bf Model\,Container} for exploration of a model:

\begin{verbatim}
  \$ genesis-g3
  Welcome to the GENESIS 3 shell
  genesis > ndf_load chemesis/cal1.ndf
  genesis > explore
\end{verbatim}
...
modelling of action potentials.  This functionality is typically
required for the efficient simulation of network models where action
potentials generated at the soma of one neuron are translated to a
discrete event that is delivered to the post-synaptic targets of the
connected neurons.

{\bf DES} queues the generated events and delivers each to the
appropriate targets respecting the connectivity of the model.  This
implies that {\bf DES} contains a subcomponent for event queuing as
appropriate to model axonal action potential propagation and contains
a subcomponent for the event distribution that includes the
connectivity matrix.

{\bf DES} is capable of associating 'secondary' parameters with each
connection, such as propagation delay, synaptic weight, or other model
specific parameters.

From the viewpoint of the CBI architecture, the {\bf DES} event queuer
is a separate solver that simulates action potential propagation in an
efficient way.  The event distributor connects different solvers that
participate in the simulation and is part of the CBI {\bf
  Communication Infrastructure}.  Splitting this functionality from
the rest of the simulator facilitates customization for the modeling
of learning rules, including spike timing dependent plasticity, and
synaptic diffusion and spillover.


\subsection{Addressing Scheme Algorithm}

The different solvers contributing to a multi-scale simulation can
hold equivalent variables.  These equivalent variables exist because
...
During a simulation, the addressing scheme algorithm enables the {\bf
  Solvers} and the {\bf Communication Infrastructure} to relate their
equivalent variables these components contain.  As this happens during
...
``experimental protocols'' (e.g. stimulus and output specifications) and connections to the {\bf Solvers}.  The addressing scheme is
...
addressing of model components', their parameters and their variables.
This syntax allows a user to express the address of a variable, for
instance for the address of the $\mathrm{Ca}^{2+}$ concentration in a
segment with name ``soma'' is:
\begin{verbatim}
  /soma/Ca->conc
\end{verbatim}

The {\bf Model\,Container} translates these symbolic references to
simulation wide unique tuples of the form (integer, type) where the
integer identifies the physical process (the $\mathrm{Ca}^{2+}$ pool
in this case) and the type identifies the concentration level.
...
to obtain a solver instance internal ID for directly addressing the
...
This algorithm allows solvers to communicate through tuples of the
form (solver instance, internal ID) and it removes the requirement for
...
simulation construction time, but not during simulation run-time.  The
second step is free of symbolic references and thus does not need the
{\bf Model\,Container}.  However it may be required during simulation
...
the model-container.  The third step of the addressing algorithm is
low-level and has been implemented in the solvers.  This has the
advantage that the model-container does not have to be in memory
...
The association between a given model and its solvers can be configured by the user.
...
\begin{verbatim}
  genesis > solverset "/Purkinje => heccer"
\end{verbatim}
...
biochemical pathways, in this example called {\it cal1}, a user would
...
\begin{verbatim}
  genesis > solverset "/**/cal1 => chemesis3"
  genesis > solverset "/Purkinje/**[!cal1] => heccer"
\end{verbatim}
...
Network modelling was previously possible in G-2. Multi-scale network
modelling of individual neurons In G-3 is also solved with different
{\bf Heccer} instances.  The {\bf SSP} software component defines
several software regression tests with network models.  One of the
network models used in this test has a ``source'' neuron that during
the simulation delivers a spike to two ``target'' neurons.


For example, projections are solved by {\bf DES}, where the {\bf
  G-Shell} syntax is:
\begin{verbatim}
  genesis > ndf_load tests/networks/spiker4.ndf
  genesis > solverset "/network/target1 => heccer"
  genesis > solverset "/network/target2 => heccer"
  genesis > solverset "/network/Purkinje => heccer"
  genesis > solverset "/network/projection => des"
\end{verbatim}
...
Simulation provides a framework to organize our understanding of biological systems. Historically, the development of neuronal simulation software for the construction of morphologically detailed neuron models and small networks has been instigated by research projects that specifically addressed complementary technical and scientific questions \cite{Moore:2010vn}. These software systems have been both highly successful and continued to grow in complexity through cycles of research project extension. However, more than twenty years of extending their functionality, usually by the direct incorporation of source code into the core of the simulator, has led to code structures so complicated that it has become increasingly difficult, if not impossible, to easily continue this process. Ultimately, the resulting stand-alone applications have become `monolithic'. That is, it has become increasingly difficult for users and developers to maintain and extend them, with the logical consequence that user workflows are often similarly degraded. A significant consequence of this process is that the development of an optimized simulation can be a considerable challenge for a neuroscientist unfamiliar with the necessary mathematical and computational theory.

One significant reason for this situation is that
 no distinction is made between biological data, numerical data, and control
operations. In practice, the result is that the complexity and monolithic nature of the software
results in a non-scalable software architecture \cite{jaeger02:_comput_neuros_realis_model_exper}.

 The CBI architecture was designed and developed in an effort to resolve the complexities associated with continual addition of functionality to simulators. As illustrated in Figure \ref{fig:cbi-architecture-simple}, the relationships between simulator control and data modules in such a federated architecture are complemented by the relationship between high level biological concepts and their mathematical implementation.
 
 According to principles of systems theoretic analysis (see \cite{Bertalanffy:1973zr,Heylighen:2006vn}), it is clear there are numerous phenomenological approaches to the development of multi-scale models suited to the transformation of a mental model into a computational simulation. Figure\,\ref{fig:mental-model-simulation-path} illustrates a selection of some of the possible steps, levels, and components of this process. Interestingly, two possible multi-scale models have been in widespread use but not generally recognized for their multi-scale characteristics: (1) Biologically inspired, e.g. a model is specified as a network and a population of detailed channel models, and (2) Mathematically inspired, e.g. use continuous cable equations to model a neuron, discrete event models for axonal propagation, and Hodgkin-Huxley equations for the channels.

Many so-called second\marginpar{where is this defined?} generation simulators remain primarily focused on the biological and mathematical aspects of multi-scale modelling. It is likely that because they do not respect all three of Marr's levels, the complexity of multi-scale simulation is greatly increased. Following the recent reconfiguration of the GENESIS simulator it is now possible to incorporate Marr's third level of `hardware', as software functionality, to give a comprehensive multi-scale simulator. In this approach, it is the implementation of a model for simulation or otherwise, that is contained by a single software entity (in this case a single `model-container') and that employs the solvers necessary to implement different mathematical tools that communicate with each other at run-time using a communication infrastructure provided by the CBI architecture that solves the problem of creating a software platform that efficiently supports the simulation of multi-scale models\marginpar{long sentence}.
...
The definition of multi-scale modelling arrived at in this paper
