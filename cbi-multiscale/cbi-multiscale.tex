...
\subsubsection{Overview of a NDF File}
...
A NDF file contains four sections. They are not necessarily
filled, but must be present in the given order.  A NDF file has
...
The header must
always be present and start with the interpreter sequence
...
This section uses the {\tt FILE} keyword to declare
...
These are models that are private to the NDF file
...
These models are visible to the NDF files into
...
One important feature of a NDF file is that it can contain
...
This supports the modular construction of a model as a given NDF file
can depend on other NDF files, i.e. the models declared in a
