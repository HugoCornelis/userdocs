...
\subsubsection{The Model Containers}
 ...
 {\bf Biology\,Model\,Container} stores biological entities and end-user
concepts and provides a solver-independent internal storage format for
models.  It also annotates the structures and concepts of a
model with biological, physical, and mathematical quantities. By `containing' the biological model, the {\bf
Biology\,Model\,Container} makes the implementation of the numerical core of
a solver independent of software implementation.
...
The {\bf Biology\,Model\,Container} supports tokens for: Grid3D, Populations, and Networks; VolumeConnect, Individual Connections; Single Cells and Morphology; HH-Channels and Synaptic (NMDA / AMPA) Channels, and Reaction--Diffusion.
...
\subsubsection{Overview of an NDF File}
...
The header section
must starts with the Unix {\it interpreter} sequence
giving the system specific absolute path to the NMC executable (for example):
\begin{verbatim}
    #!/usr/local/bin/neurospacesparse
\end{verbatim}
...
(public) components.  This supports the controlled and modular construction of a multi-scale model as a given {\bf }NDF file
can depend on other NDF files.  
...
or have a function value.  For example:
