...
If the ``{\tt set\_verbose information}'' flag has been set. the GENESIS shell would respond with
\begin{verbatim}
    create_cell: /n
\end{verbatim}
...
Importantly, remember that GENESIS assumes parameter values are in SI units, which in the case of $C_M$ is
Farads/$m^{2}$, for $R_M$ $\Omega \cdot m^{2}$, and for $R_A$, $\Omega \cdot m$/ GENESIS is able to
