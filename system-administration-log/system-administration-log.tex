...
\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 1/8/2013
  \item[] {\bf Actions:} Did a manual workspace update and install of the developer package. 
  \item[] {\bf Reason:} Cron script was in a state that prevented it from being executed. 
...
\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 1/2/2013
  \item[] {\bf Actions:} Manually restarted all monotone servers. Added the monotone user to the sudoers file. 
  \item[] {\bf Reason:} System was brought down over the holidays and restarted. Some servers did not start up when rebooted.
    \item[] {\bf Impact:} Tester is currently halted due to a bug in the Developer package. Looking for a fix.
\end{itemize}

\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 5/26/2012
  \item[] {\bf Actions:} Checked kernel and other system logs for any errors.
  \item[] {\bf Reason:} Was informed by David that the repo server has a warning light flashing in the server room. Doesn't appear to be affecting any of the cron jobs or other functions.
  \item[] {\bf Impact:} Tester should proceed as normal.
\end{itemize}
...
\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 4/6/2012
  \item[] {\bf Actions:} Installed the Mo::builder perl module
  \item[] {\bf Reason:} Possibly deleted this by mistake after removing some junk perl modules that were unreachable.
  \item[] {\bf Impact:} Tester should proceed as normal.
\end{itemize}
...
\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 4/5/2012
  \item[] {\bf Actions:} Performed calls to the perl config to see the values of the installsitelib, and siteprefix variables in the perl Config module.
  \item[] {\bf Reason:} Fedora seems to have an incompatible site prefix but it works fine on Debian/Ubuntu.
    \item[] {\bf Impact:} Tester should proceed as normal.
\end{itemize}
...
\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 3/21/2012
  \item[] {\bf Actions:} Reactivated package builds.
  \item[] {\bf Reason:} Packaging was disabled previously due to an error with reverting the tag updates.
    \item[] {\bf Impact:} Tester should proceed as normal.
\end{itemize}
...
\item
  \begin{itemize}
  \item[] {\bf Admin:} Mando
  \item[] {\bf Date:} 3/20/2012
  \item[] {\bf Actions:} Performed a revert across all packages in the tester project directory. Disabled all package building.
  \item[] {\bf Reason:} Discovered that the neurospaces\_revert was not working correctly because it wasn't changing directories. After fixing the script a proper revert was performed. 
  \item[] {\bf Impact:} Tester should proceed as normal, will monitor for any other errors. If no other errors occur the package creation will be reenabled.
\end{itemize}
...
\item[] {\bf Admin:} Mando
\item[] {\bf Date:} 4/5/2012 
\item[] {\bf Actions:} Checked the install path for perl using the Config module.
\item[] {\bf Reason:} Perl is attempting to install to a path that centOS does not search for perl modules in. It seemed to work previously possibly due to modules that were in the correct path remaining from previous runs.
\item[] {\bf Impact:} Website should continue running.
\end{itemize}
