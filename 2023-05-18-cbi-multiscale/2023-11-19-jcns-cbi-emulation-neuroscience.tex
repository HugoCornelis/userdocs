...
\section*{Abstract}
To better understand both the development and implementation of a realistic scale-independent neural simulator, it has been found necessary to abandon central tenets of the biological sciences and computational neuroscience in particular. The first originates with Plato and Aristotle and was more completely articulated by the Neo-Platonists. Initially referred to as \textit{The Celestial Hierarchy}, it elaborated a metaphysic that has survived over 2,000 years including both the Scientific Revolution and the Age of Enlightenment. During that period it was reconfigured as \textit{The Great Chain of Being} and continues as a core secular belief and organizing principle for Western civilization, surviving as a subtle yet powerful driver underpinning both Western religious and scientific interests and activities. Thus, by its ambit, it remains a driver of contemporary scientific doctrine. It is the notion of hierarchy which, together with its constituent levels and reductionist and mechanistic assumptions, when combined with computer modelling, evolves monolithic software architectures that concomitantly expose the multiscale problems which have evolved to confound computational implementations of cognitive models in theoretical neuroscience.

A deeper understanding of the Computational Biology Initiative Federated Software Architecture at the core of a reconfigured GENESIS software platform has revealed the origin of these two implementational problems. They arise in the presence of an insufficiently principled and complete separation of concerns during the development of neurobiological simulator architectures. Here, an outline is given that describes a new computational synthesis based on clear demarcation between cognitive models and their computational implementation. This facilitates escape from the consequences of the truncated nature of linear scientific workflows.

A principled separation of concerns based on more than 35 years experience with GENESIS development, when associated with a greatly improved understanding of the relations between cognitive models and their computational counterparts, reveal increasingly self-evident solutions. The scale-independent CBI Federated Software Architecture obviates otherwise significant implementational problems. With this regonfigured approach, problems are eliminated as cognitive models that better emulate anatomical architectures achieve increasing concordance with their computational implementation.
...
It has also been recognized that the relationships between the biological levels in a hierarchy, as might be thought to exist in Fig.\,\ref{fig1}, is a crucial aspect of a ``total picture." However, it is one that is frequently neglected~\cite{bickle19} despite being the “glue” that binds knowledge of neuronal activity to subcellular and molecular mechanisms “below” and circuit, network and systems activity “above." This problem is particularly evident in attempts to relate “cognitivist” psychological theories, postulating information-bearing representations and processes operating over their contents, to neuronal activity at the level of chemistry and physics. In summary, any “co-evolution” between these explanatory levels still appears to be more a distant goal than an operative methodological principle guiding day-to-day integrative research in neuroscience. 
...
Addressing multilevel concerns through computational modelling, a methodology applied successfully in more developed physical sciences; the motivating hope has been to address the interlocking levels of theory and explanation in the mind/brain through similar methodologies, thus achieve comparable results~\cite{bickle19}. In short, this paradoxical situation appears to leave neuroscience and particularly theoretical neuroscience bathed in its own form of ``pre-Copernican" miasma.

\subsubsection*{Is Simulation Neuroscience the Solution?}
\label{subsection:simneuro}

In redefining the field of Simulation Neuroscience, Fan and Markram~\cite{fan19} consider that its deepest meaning consists of constructing and simulating a brain from the most fundamental principles that can be isolated. Their aim is to understand and link the multiple layers forming the human brain (more psychologically, in their own word ``ourselves'') from molecules and cells to brain function. They consider the philosophy of this approach originates from the will to transcend the barriers of scale and complexity during the evolution of neuronal mapping, connectivity mapping and functional mapping for the purpose of giving meaning and life to data and theories.
...
This is particularly the case as despite early realization of a relation between components and physiology~(see, for example \cite{sieck17}), there is neither a clear understanding of the relations between structure and function in the central nervous system nor a detailed understanding of their relation to behavior.

\subsubsection*{Workflows as Instigators for the Dissemination of Scientific Knowledge}

Successful accumulation of knowledge is critically dependent on the ability to verify and replicate every part of scientific activity. To this end, workflows help with the planing and monitoring of projects in a methodical and logical way.  However, such principles are difficult to enact when researchers continue to resort on {\textit{ad-hoc}} workflows with a poorly maintained code base~\cite{kubilius13}. A significant problem has also continued to be related to the fact that workflows typically appear to be incomplete due to their linearity (see for example~\cite{eriksson22,stockton15}). Although more comprehensive and detailed workflows have been published by larger research groups, when taken collectively, their single greatest shortfall remains their primarily linear nature (see for example~\cite{levenstein23,fan19}.

An alternative view is provided by Rosen. It concerns the extent of the scientific method and the fact 
%\marginpar{\scriptsize do we need a citation for this fact?  Or is this still from Rosen?  In the latter case, the citation is at the wrong place.}
that experimentalists tacitly feel that science is consubstantial with laboratory practice~\cite{rosen96}. This so-called consubstantiality is an argument in support of ``closing the workflow loop" between modeling and experiment. The model container defined within the CBI Simulator Architecture enables replication of the cyclical nature of nervous system structure embodied in reentrant connectivity~\cite{edelman13} and (for example) the apparent reciprocal connectivity both within the visual cortex and between that area and other brain regions~\cite{vanessen93}.

In short, a linear workflow implies (as mentioned in relation to Fig\,\ref{fig1} above), that a modeling project is initiated \textit{de\,novo}, either by a research group, laboratory, or individual and then pursued until completion. Under such circumstances, with the benefit of hindsight, it often becomes an open question as to how closely a  project has actually followed a given workflow. Further, at the end of a linear workflow and thus project, it is almost a certainty that there is no indication of how it might best be continued. This is particularly the case when all or even part of a research team also changes and a new team or individual is uninformed concerning relevant previously acquired knowledge that is nevertheless critical to further timely development and research progress.

Alternatively, in the presence of a "closed," "cyclic" or "iterative" workflow, a project can efficiently be suspended. However, the significant advantage is that at any step of a workflow there is ongoing specification of how to best continued. In this process, a model can always be iteratively elaborated by incorporation of new insights into both the cognitive and implementational models. The benefit is that the underlying workflow does not necessarily have to be reconfigured.

 A further advantage of an iterative workflow is seen with regard to publication. Unlike the case of a linear workflow where a pause at any workflow step typically halts the workflow until the step is completed, in an iterative workflow it is trivial to insert publication pathways that do not interfere with ongoing model development and implementation and allow the "spin off" of appropriate timely publication.
...
% This observation raises questions such as, ``What is/are the organizational principle(s) of the nervous system?'' and ``To what extent do current theories retard progress due to endemic beliefs?'' The pragmatic answer given by Jacobson might be that in the process of making models, it is the model maker who chooses the scale and level of the model based on predetermined cultural conventions used to represent things and events, structures and functions. 
...
Later, the human brain was described by Sherrington first as an ``enchanted loom" and later as a telephone-exchange with switches.  However what was wanted really of the brain was, it would seem, were the subscribers using the exchange~\cite{sherrington53}.
...
\bibliography{../tex/bib/2023-11-11-g3-refs-adc}
