...
Here voltages are measured with respect to a resting potential of zero (rather than the actual value of about -70 mV), and are given in $mV$. With these expressions for the rate constants, one can numerically solve Eqs.~\ref{eq:eq6},~\ref{eq:eq9}, and~\ref{eq:eq10} for the time dependence of the activation variables, and use these in~\href{../compneurisci-1/compneurosci-1.pdf} {Eqs. 2} and~\href{../compneurisci-1/compneurosci-1.pdf} {5} to solve for the conductances. These can then be used in \href{../compneurisci-1/compneurosci-1.pdf}{Eq. 1} to solve for the time behavior of the membrane potential.
