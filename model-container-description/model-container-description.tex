...
Inspection of graph structure and parameters is done a small
command-driven query engine called the {\bf querymachine}.  The
initial purpose of the {\bf querymachine} was to debug the
model-container with an automated regression tester, but it is also
useful for 'gluing' with scripting languages such as Python and Perl.
The {\bf querymachine} supports completion on command names and node
labels (via the GNU readline library), but does not allow
sophisticated queries via logical connectives or aggregates as used in
relational and XML databases.  The hierarchical of a stored
neuroscience model can be addressed in a hierarchical way\:: the node
...
example, if the current symbol is \texttt{Ca\_pool}, the redirection
of \texttt{.}  gives simply \texttt{Ca\_pool}.

The\marginpar{replace with CBI architecture} complete software design
overview is depicted in figure \ref{fig:neurospaces-overview}.  To the
left is a specification of the model in an NDF formatted file.  A
lexical analyzer transforms the file to a stream of tokens that is
parsed by a syntax analyzer.  The syntax analyzer communicates the
semantics of the data stream to the model-container using via an
Application programmer's interface (API).  The biological component in
the data stream that is beind specified, is called the ``active
symbol''.  Specifications that cannot be represented in a declarative
manner employ a builtin extensible library of algorithms.  Algorithmic
specifications of, or changes to a component are specified with tokens
that reference the algorithms in the library by name (see
section~\ref{sec:neurospaces-loadable-modules} and
section~\ref{sec:neurospaces-module-instances}).  The algorithms are
activated by the parser and interact with the model-container via its
API to get a reference to the active symbol.  After an algorithm has
been active the resulting token stream is stored by the
model-container and available through its API as a declarative
mathematical specification of the model to be simulated.  The API
allows further validation or graphical output of (parts of) the model
and at the same time allow solvers\footnote{'Solvers' should be
  interpreted in a broad sense here, i.e. a solver is anything that
  communicates with the model-container to inspect a model.  In this
  context a graphical user interface is similar to a solver.} to
inspect what the mathematical equations they have to solve.
%  The
%numbering scheme of the forestspace is used to find computed variables
%and to set up communication between the solver instances.  Because the
%(forestspace) links are always in the discrete event domain, the
%communication between solvers is currently restricted to discrete
%events only.
