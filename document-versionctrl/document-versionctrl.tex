...
The following sections give example workflows for monotone use. They may be consulted on the basis of the above figure. This figure shows the relationship between a local workspace, its associated monotone repository, and how after an initial ``check in'' (ci), a document can be ``pushed'' to the monotone server database that forms a monotone repository. Note that documents are private to the local account until they are pushed to the monotone server.

Once a document is located on the monotone server repository, it is available to be published as part of the GENESIS documentation.

\subsection*{Updating the Local Workspace}
Updating your local workspace begins with determining which files are registered with monotone. To do this your local monotone repository can be queried to generate a list of any new files that may have been added to your local work space since the last update
\begin{verbatim}
    mtn ls unknown
\end{verbatim}
...
\begin{verbatim}
    ...
    mtn add --unknown
\end{verbatim}
These commands add the files discovered in your local workspace to your local monotone repository. That this has occurred correctly can be checked with
\begin{verbatim}
    mtn ls known .
\end{verbatim}
...
\begin{verbatim}
    mtn ls missing
\end{verbatim}
You can then drop any missing files from your local repository by entering:
\begin{verbatim}
    mtn drop --missing
\end{verbatim}
...
\begin{verbatim}
    mtn ci
\end{verbatim}
You can automate the synchronization of your local data base with the monotone repository on the server by placing an executable file (named for example,\,{\it userdocs-sync}) in a bin directory in your local work space, e.g. {\it neurospaces\_project/userdocs/source/snapshots/0/bin}. 
...
./bin/userdocs-sync
\end{verbatim}
updates the monotone repository on the server. Once this has been done, the files are available for other registered users to pull down to their own local monotone repository to check them out to their local workspace.
